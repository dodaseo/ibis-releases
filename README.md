...
- **주소 한 줄로 온라인 자료·신문기사**(2026.9.4): DOI·ISBN 을 넣던 칸에 기사 주소를 붙여넣으면 제목·매체·저자·게시일을 채우고 검색일을 오늘로 넣습니다 — 표준 정보가 없는 페이지는 본문을 Ibis AI 가 읽습니다
...
- **서재(서지·첨부)** — 새로 설치하면 `문서\Ibis` 에 생깁니다(2026.8.10 부터). **쓰고 계시던 분의 서재는 옮기지 않고** 예전 자리를 그대로 씁니다. 서재 위치·새로 만들기·열기·사본 저장(백업)은 **⚙설정 › 라이브러리** 에서 합니다.
- **API 키·AI 설정·테마·글꼴·검색 색인** — 이 PC 전용으로 `%LOCALAPPDATA%\Ibis` 에 남고, 어떤 동기화에도 실리지 않습니다.
...
최신 기록 **2026-09-04** · 누적 합계 **363건** (설치 17 · zip 48)
