...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.18**.
...
- **첨부 PDF 본문까지 (2026.9.5)** — '전체' 검색이 첨부 PDF 의 글자까지 봅니다. 제목·초록에 없는 낱말이 PDF 에 있으면 그 문헌이 뒤에 추가되고 행 아래 **발췌 한 줄**(일치 부분 강조)이 보입니다. 고급 검색에 **본문(PDF)** 필드가 생겼습니다. 색인은 첨부 뒤 배경에서 저절로(부팅 뒤·첨부 뒤 1분 안) 만들어지고, 상태와 [지금 색인]은 `⚙설정 › 서재 › PDF 본문 색인` 에 있습니다. 스캔본은 글자 인식 뒤에 잡힙니다. 색인은 낮은 우선순위로 돌고 상태줄에 `n/전체` 진행이 보입니다. 2026.9.16 부터는 누르거나 입력하는 동안에는 다음 파일을 열지 않아, 색인 중에도 앱이 바로 반응합니다.
