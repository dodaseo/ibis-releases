...
| **사이드바** | 라이브러리 · 컬렉션 · 스마트 컬렉션 · 문서 연동 · 기타(미분류·중복 점검·휴지통·연결 안 된 PDF) |
...
- **방법 B** — `⚙설정` 의 'DOI로 추가' 칸 → 해석 후 바로 내 서지에 보관.
...
- 경로: `⚙설정` › '가져오기 (RIS·BibTeX·CSL)' → 파일 선택(복수 가능).
...
- `＋추가` 창의 붙여넣기 상자에 텍스트를 붙이거나 파일을 끌어놓기.
...
| 제공자 | 유형 | 필요 조건 |
|---|---|---|
| Claude Code | 구독(키 불필요) | CLI 설치 + 1회 로그인 |
| Antigravity(agy) | Google 구독(키 불필요) | CLI 설치 + 1회 로그인 |
| Codex | ChatGPT 구독(키 불필요) | CLI 설치 + 1회 로그인 |
| Anthropic API | API 키 | console.anthropic.com |
| OpenAI API | API 키 | OpenAI |
| Gemini API | API 키 | Gemini |

- **모델(선택)** — 비우면 CLI 개인 기본값 / API 내장 기본값(Anthropic `claude-haiku-4-5` · OpenAI `gpt-4o-mini` · Gemini `gemini-2.0-flash`).
- **폴백(선택)** — 1차 실패 시 지정 제공자로 1회 자동 재시도.
...
`⚙설정`(톱니)과 `🔧도구`(도구 아이콘)에서 관리합니다. 설정 칸은 대부분 **'빈 칸은 유지' 규칙**이라 비우고 저장해도 지워지지 않습니다(삭제는 '지움' 버튼).
