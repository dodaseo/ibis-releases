...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.19**.
...
- **설치 위치를 묻지 않습니다**(2026.8.10 부터) — `%LOCALAPPDATA%\Programs\Ibis` 에 설치됩니다. 인앱 업데이트가 이 폴더를 직접 교체하므로 쓰기 권한이 있는 위치여야 합니다.
...
1. **`1 · 서재 위치`** — 기본은 `%LOCALAPPDATA%\Ibis`. 클라우드 폴더로 지정하면 여러 PC 동기화.
...
| **레일**(맨 왼쪽) | 서재 · **찾기(학술)** · **법·정책**(저울) · **통계**(막대) · 그래프 뷰 전환(2026.9.12 — 찾기 셋은 *어디서* 찾을지고, 낱말/문장은 찾기 줄의 **키워드 \| 질문** 칩). 찾기 모드가 **같은 서재 사이드바**를 쓰고(어느 모드에서든 컬렉션을 누르면 그 뷰로 갑니다), 찾기 줄과 그 아래 DB 체크 행도 같은 자리입니다(2026.9.8). 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래). 참고문헌 갱신은 [4.5](#45-전체-갱신--참고문헌-갱신) 의 자리에서 합니다 |
| **사이드바** | 맨 위 **'새 문헌 추가'** 버튼(추가 방법 메뉴 런처) + 서재 · 컬렉션 · 스마트 컬렉션 · 문서 연동 · 기타(미분류·중복 점검·휴지통·연결 안 된 PDF) |
...
- **본창의 ✕ 와 Alt+F4 는 작업표시줄로 내립니다**(2026.9.18) — 뒤에서 도는 일이 끊기지 않게. 앱을 끄는 것은 `파일` › **종료** 입니다. 별도 창(설정·PDF·상세)의 ✕ 는 그 창만 닫습니다.
- **서재가 바뀌면 화면이 바로 갱신됩니다**(2026.9.18) — 서지·태그·AI 노트·컬렉션이 바뀌면 클릭 없이 목록·상세·사이드바 건수에 반영됩니다. 입력 중이면 상세는 손을 뗀 뒤 반영되고, 검색·질문 결과 화면은 덮지 않습니다.
- **메뉴바와 `⚙설정` 의 경계** — 메뉴바에는 *지금 하는 일(동사)* 이, 설정에는 *한 번 정해 두는 것* 이 있습니다. `파일` = 가져오기·내보내기, `편집` = 서지 편집(F2)·원문 확보·모두 선택(Ctrl+A), `도구` = **문서 / 서재 정리(중복·원문·AI 태그·첨부 이름) / 보기** 세 구획(2026.9.5 재편 — 제출용 사본 만들기는 원고 이어받기 옆), `⚙설정` = 서재 위치·API 키·AI·클라우드([9절](#9-설정api-키)).
...
- `⚙설정` 의 '연결' 버튼도 같은 동작.
- 앱은 독립 모드로 시작하고 문서에 자동으로 연결하지 않습니다(2026.9.4 부터). 연결은 위 절차대로 직접 고릅니다.
...
연구 질의를 문장으로 입력하면 국내외 학술 DB 를 검색해 관련도 순으로 보여 줍니다. 레일 **찾기(학술)** 아이콘을 누르고 찾기 줄 왼쪽의 **[질문]** 칩을 켭니다(2026.9.12 — 별도 아이콘이 아니라 키워드/질문 **모드 칩**입니다. 통계 아이콘에서도 됩니다 — [3.2-1](#32-1-통계kosis--표-찾기--수치-보기--데이터세트로-인용-2026912); 법·정책은 키워드뿐).

결과는 키워드로 찾기와 같은 표에 나옵니다. 맨 왼쪽에 **관련도 칸**(순위 · 막대 · 0~1 점수)이 하나 더 붙고, 그 칸은 질문으로 찾기에서만 보입니다. 최근 질의와 사정 고지(잘린 것·빠진 것·실패)는 표 바로 위 한 줄에 있습니다. 질의에서 읽어 적용한 조건도 여기 **조건 …** 으로 적힙니다.

![질문으로 찾기 — 문장으로 물으면 관련도 순위로 돌려준다](images/ask.png)

1. 질의를 문장으로 적습니다 — 예: `국내 교육사회학 연구에서 교육불평등을 어떻게 다뤄 왔나`.
2. 찾기 줄 아래 **체크 행**에서 어디서 찾을지 고릅니다: `전체` │ **내 서재** │ **국내**(KCI·ScienceON·국립중앙도서관) │ **국외**(OpenAlex·Crossref·ERIC·arXiv·PubMed·S2). 행 오른쪽 끝에 지금 조합(`KCI·ScienceON·국중도 외 3에서 찾기`)과 **예상 크레딧**(`+2~7` 처럼)이 적힙니다. **키워드로 찾기와 같은 목록·같은 기억**입니다(2026.9.12 — 한쪽에서 끄면 다른 쪽도 꺼져 있습니다).
   - **내 서재 칸은 기본이 꺼짐**입니다(2026.9.10) — 꺼져 있으면 DB 에서만 찾고, 그중 이미 담아 둔 것에는 **[보관됨]** 배지가 붙습니다. 켜면 서재 문헌도 후보로 순위에 함께 듭니다. 키워드로 찾기에도 같은 칸이 있습니다(2026.9.12 — 켜면 서재에서 같은 검색어로 찾은 문헌이 [보관됨]으로 함께 서고, 서재만 켜면 서재 검색).
   - **법·정책 다섯 갈래는 여기에 없습니다**(2026.9.11) — [3.2](#32-검색-소스) 의 키워드로 찾기에서만 씁니다. 갈래마다 채점에 쓸 본문 길이가 크게 달라 순위가 치우치기 때문입니다.
3. 진행이 단계로 보입니다 — `검색어 만들기 → DB에서 찾기 → 관련도 매기기 → 정리`(지금 단계가 점멸).
4. 최근 질의는 셋만 보이고 나머지는 **[+N ▾]** 에 접혀 있습니다 — 누르면 **크레딧 0** 으로 그때 결과가 되살아납니다.

- 검색어 생성과 관련도 채점은 AI 가 합니다. 낱말을 직접 고르지 않아도 됩니다.
- 모은 후보(보통 200여 건)를 전부 채점해 100여 편을 순위로 보여 줍니다.
- 국립중앙도서관 단행본도 순위에 오릅니다. 책소개가 없으면 카카오 책·알라딘에서 가져와 채점합니다.
- **크레딧을 씁니다**(Ibis AI 를 쓸 때) — 검색어 만들기와 채점에 들어갑니다. 체크 행 오른쪽 끝에 **예상 크레딧**이 미리 뜨고(한글 질의는 검색어 생성이 붙어 `+2~7`), 남은 양은 화면에 표시됩니다.
- 이전 결과는 최근 질의 목록에서 누르면 크레딧 소모 없이 다시 표시됩니다. 외부 문헌 행을 누르면 상세가 열리고 거기서 내 서재에 담습니다.
- **저자·학술지만 묻는 질문에는 관련도가 없습니다 (2026.9.13)** — "양연동의 연구물을 찾아줘" 처럼 주제 없이 사람이나 학술지만 물으면 **저자 검색**으로 가고, 관련도 칸 대신 **최신순**으로 섭니다. 관련도는 주제의 일치 정도를 재는 값이라 저자 검색에는 맞지 않기 때문이며, 채점을 건너뛰므로 크레딧도 들지 않습니다.

- **질의에 적은 조건을 읽습니다 (2026.9.3)** — `2015년 이후`·`학위논문만`·`OO 저자`·`OO학회지`·`양적 연구는 빼고` 처럼 문장 안에 넣은 기간·유형·저자·학술지·제외·선호가 그대로 DB 에 보내는 검색 조건이 됩니다. 따로 칸을 채우지 않아도 됩니다.

- **사이드바를 누르면 질문 화면을 닫고 그 뷰로 갑니다 (2026.9.6)** — 키워드로 찾기와 같은 동작입니다. 결과는 남아 있어 돌아오면 그대로이고, 결과 행을 컬렉션에 끌어 넣는 것도 그대로입니다.
- **여러 개 골라 한 번에 (2026.9.5)** — Ctrl 클릭으로 더하거나 빼고 Shift 클릭으로 범위를 고른 뒤 끌면 고른 전부가 컬렉션으로 갑니다(서재 목록과 같은 규칙).
- **'많이 인용된·대표적인·고전적인' 조건이 순위에 실제로 반영됩니다 (2026.9.5)** — 해외 DB(OpenAlex·Crossref)의 피인용 수를 새로 담고(상세의 '피인용(해외 DB)' 줄) KCI 피인용과 따로 정규화합니다. 관련도가 분명히 낮은 문헌이 피인용만으로 앞서지는 않습니다.

> 낱말로 찾는 것은 **키워드로 찾기**([3.2](#32-검색-소스))입니다. 질문으로 찾기는 무엇을 찾아야 할지부터 넓게 볼 때 씁니다. 낱말로 찾으면 키워드로 찾기, 문장으로 물으면 질문으로 찾기입니다.
>
> 두 찾기는 **같은 자리(찾기 줄 아래)에서 같은 방식**으로 DB 를 고르고, 2026.9.12 부터 **목록도 기억도 같습니다**(내 서재 칸·S2 둘 다 양쪽에). 법·정책은 별도 아이콘(레일 셋째)이고 키워드로만 찾습니다. 물음표로 끝나는 검색어에는 질문 모드를 권하기만 하고 자동으로 바꾸지는 않습니다(크레딧이 들기 때문입니다).

### 3.2 검색 소스

레일의 찾기 아이콘 셋이 **어디서 찾을지(범위)** 입니다(2026.9.12): **찾기(학술)** — 체크 행 `전체 │ 내 서재 │ 국내 KCI·ScienceON·국중도 │ 국외 OpenAlex·Crossref·ERIC·arXiv·PubMed·S2`(arXiv·PubMed·S2·내 서재는 **기본 꺼짐**) / **법·정책**(저울) — `법령·판례·정책자료·법률안·회의록` / **통계**(막대) — KOSIS 하나라 체크 없이 안내 한 줄. 범위가 갈려 있어 법령 몇 건 사이에 논문 수백 건이 끼지 않고, 조합은 **범위마다 따로** 기억됩니다. **'국내'·'국외' 글자를 누르면 그 묶음이 통째로 켜지고 꺼지며**, 원하는 대로 섞어도 됩니다(KCI+ERIC 만 같은 조합도 가능). 행 오른쪽 끝에 `○○에서 찾기` 라고 지금 조합이 적힙니다(고른 것이 없으면 'DB 를 하나 이상 고르세요'). 낱말로 찾을지 문장으로 물을지는 찾기 줄 왼쪽의 **키워드 \| 질문** 칩입니다.

소스는 **국내 3종 · 해외 6종 · 법·정책 5갈래** 이며, 둘 이상을 켜면 병렬로 질의한 뒤 중복을 합칩니다.

> **법·정책을 켜면 학술 DB 는 함께 검색하지 않습니다**(2026.9.9). 결과가 섞이면 구분해 찾는 의미가 없기 때문입니다. 학술을 켜면 법·정책이 꺼지고, 그 반대도 같습니다.
> **법·정책은 칸 하나가 한 인용 갈래**입니다(기관 이름이 아니라 인용할 때 부르는 이름). 담으면 바로 참고문헌 서식이 나옵니다 — `교육부 (2026). 초ㆍ중등교육법. 법률, 제21659호(시행 2026. 8. 20.).`
> **관련도순만 고를 수 있습니다**(두 API 모두 정렬 인자를 받지 않습니다).

| 묶음 | 소스 | 커버리지 | 키 |
|---|---|---|---|
| 국내 | **KCI** | 국내 학술논문 | **불필요(앱 내장)** |
| 국내 | **ScienceON** | 국내 학술·학위논문(KISTI) | **불필요(앱 내장)** |
...
| 해외 | **PubMed** (2026.9.5) | 의학·보건·간호(미국 국립의학도서관) — 초록·MeSH 주제어 포함, 총건수 표시. 주제 범위가 좁아 **해외 통합·질문으로 찾기에서는 기본 꺼짐** | 불필요 |
| 법·정책 | **법령** · **판례** | 법제처 국가법령정보 — 현행법령·자치법규, 대법원·하급심 판례 | **불필요(앱 내장)** |
| 법·정책 | **정책자료** · **법률안** · **회의록** | 국회도서관 — 공공기관 정책자료(회색문헌), 국회 의안, 국회 회의록 | **불필요(앱 내장)** |
| — | 내 서지 | 보관한 문헌 — 키워드·질문 두 찾기 모두 **'내 서재'** 칸이 있습니다(2026.9.12, 기본 꺼짐). 켜면 서재에서 같은 검색어로 찾은 문헌이 결과에 **[보관됨]**으로 함께 서고, 서재만 켜면 서재 검색이 됩니다. 서재 모드의 찾기 줄([3.8](#38-내-서지-전문검색fts))은 그대로입니다 | — |

- **KCI 와 국립중앙도서관은 앱 내장 키로 동작합니다** — 사용자가 발급받을 것이 없고 설정에 입력 칸도 없으며, 설치 직후부터 소스 목록에 보입니다.
- **모든 학술 DB 와 법·정책이 앱 내장 키로 동작합니다**(2026.9.9). 발급받을 키가 없습니다. ScienceON 인증키 입력 칸도 없앴습니다.
- DB 가 실패하면 체크 행의 그 칸에 ⚠ 가 붙고(툴팁에 사유), 안내줄에 "이 DB 의 결과는 빠져 있습니다. 0건이어도 자료가 없다는 뜻이 아닙니다" 가 남습니다(2026.9.9). 경고는 그 DB 를 다시 불러 성공했을 때 지워지고, 체크를 자동으로 끄지는 않습니다.
- 학술논문은 KCI 가, 그 밖의 국내 발행물은 국립중앙도서관이 맡습니다. 유형을 지정하지 않은 국립중앙도서관 검색은 소장자료 네 갈래를 동시에 검색합니다.
- RISS 는 검색 소스가 아니라 **원문 확보 경로 전용**입니다([7절](#7-원문-확보pdf-뷰어)).
- 찾기: **체크 행에서 DB 고르기 → 낱말 입력 → Enter** → 페이지당 20건. 체크를 바꾸면 목록이 새 조합으로 바뀌고, 실제 검색은 Enter 로 합니다(질문으로 찾기와 같습니다).
...
![키워드로 찾기 — 찾기 줄 아래 DB 선택 행, 결과에 출처 배지](images/search.png)

### 3.2-1 통계(KOSIS) — 표 찾기 · 수치 보기 · 데이터세트로 인용 (2026.9.12)

레일의 **통계** 아이콘은 KOSIS 국가통계포털의 통계표를 찾습니다(내장 키 — 설정 없음). 표 하나가 서지 하나(**데이터세트** 유형)입니다.

- **키워드** — 통합검색은 검색어 하나·한 번에 최대 20건·쪽 넘김 없음. 더 보려면 검색어를 좁히세요. 같은 작성기관·같은 표명의 **연도별 표는 최신 표 하나로** 합쳐 보이고 다른 연도는 표 주석에 적힙니다. 목록 열은 작성기관 · 연도 · 통계표명 · 통계조사.
- **목록이 통계조사별로 묶입니다 (2026.9.13)** — 머리줄에 `조사명 · 표 수 · 작성기관`이 서고, 표마다 둘째 줄에 **수록 기간과 분류 경로**가 붙습니다. 한 조사에 표가 많을 때 무엇을 볼지 고르는 데 씁니다. 정렬을 바꿔 조사가 흩어지면 머리줄은 사라집니다.
- **질문** — "초등학생 1인당 월평균 사교육비는 얼마나 되나?" 처럼 문장으로 물으면 표를 모아 **관련도 순**으로 돌려줍니다. 상위 5개 표는 최근 시점 수치가 행에 발췌로 미리 보입니다. 크레딧은 학술 질문과 같습니다.
- **상세**(서지 상세와 다른 화면) — 표명·작성기관·조사·수록 기간 → **[수치 보기]** → 분류 경로·표 주석 → 인용 → 서지 정보. 피벗·차트는 [KOSIS 에서 보기]로, 원자료는 **[CSV 저장]**(분류 코드·이름·항목·단위·시점·값)으로 받습니다. 한 번에 4만 셀까지(KOSIS 제한) — 넘으면 시점 수나 항목을 좁히라고 알립니다.
- **수치 표는 분류·항목이 행, 시점이 열입니다 (2026.9.13)** — 오래된 시점부터 오른쪽으로 가고 첫 열은 고정돼 스크롤해도 이름이 남습니다. 기본 항목은 첫 항목(대개 '전체')이고 '항목 전체'는 조작 칸에서 고릅니다.
- **[표 복사] (2026.9.13)** — 원고에 바로 넣을 표를 클립보드에 담습니다: `<표>` 캡션(항목·단위) + **탭 구분 표** + `자료:` 출처 문장(지금 스타일의 참고문헌 항목). 한글·Word·엑셀에 그대로 붙습니다. 표 모양을 원자료 그대로 원하면 [CSV 저장] 쪽입니다.
- **담기·인용** — 담는 순간 **인출일**이 오늘로 적힙니다(수치는 갱신되므로 언제 봤는지가 서지의 일부). 인용은 `국가데이터처 (2025). 학교급별 학생 1인당 월평균 사교육비 [데이터세트]. 초중고사교육비조사. 2007~2025. KOSIS 국가통계포털. URL (인출일: 2026. 9. 9.)` 꼴이고, 영문 APA·Vancouver·IEEE·Chicago·MLA 도 됩니다. 데이터세트에는 PDF 확보·AI 노트·검토 버튼이 없습니다.

### 3.3 여러 DB 를 한 번에(병렬 + 중복 병합)

- 체크 행에서 **둘 이상 켜면** 켠 DB 를 **동시 질의**합니다. **국내**(KCI·ScienceON·국중도)나 **국외**를 통째로 켜는 것이 흔한 쓰임이지만, 조합은 자유입니다.
...
논문의 DOI 에 해당하는 단행본의 식별자는 **ISBN** 입니다. 학술 DB(KCI·Crossref)는 단행본을 거의 담지 않으므로 도서관·서점 데이터로 조회합니다(논문의 'DOI로 채우기'에 대응하는 단행본용 기능).

- **추가** — `새 문헌 추가` 창의 **식별자로 채우기** 칸에 ISBN(하이픈 있어도 됨)을 넣고 **[바로 저장]**. 조회 후 단행본 서지가 곧바로 내 서지에 보관됩니다.
...
- 전용 필드를 고르면 **위 체크 행이 그 DB 로 바뀝니다**. 그 DB 가 지원하지 않는 조건은 이유를 안내줄에 적습니다.

**전역 한정자**(행이 아니라 질의 전체에 걸립니다)

- **유형(공통)** · 정렬 · 발행년 범위. 유형에 **법령·조례**(2026.9.9)와 **데이터세트**(2026.9.12)가 있습니다. (어느 DB 에서 찾을지는 고급 검색 안이 아니라 **찾기 줄 아래 체크 행**입니다.)
- **고급 검색은 범위를 따릅니다**(2026.9.12) — 법·정책과 통계는 두 API 다 검색어 하나만 받으므로 조건 줄이 하나(필드 '전체' 고정)이고 정렬 줄은 없습니다(관련도순뿐). 통계는 유형 줄 대신 **작성기관·통계조사** 칸(받은 목록 안에서 거름)과 **수록 기간(겹침)**, 법·정책은 **연도(시행·선고·제안·회의)** 를 씁니다(통계의 연도는 수록 기간과 겹치면 걸리므로 `2015~2020` 이 1992~2026 수록 표를 제외하지 않습니다).
...
- 초록 검색과 제외(NOT)는 넣지 않았습니다. KCI 는 초록 조건을 적용하지 않고, 국립중앙도서관 응답에는 초록 필드가 없기 때문입니다. 초록은 **'내 서지'** 에서만 고를 수 있습니다.
...
- 검색 범위: 제목·저자·수록처·태그·키워드·**AI 요약(노트)**·초록·영문 병기 + **첨부 파일명**.
- 검색어 **3자 이상** → FTS5(트라이그램), **2자 이하** → 제목·수록처 LIKE 폴백.
- 고급 검색에서 필드를 좁히거나 파일명 검색이면 FTS 대신 지정 필드만 봅니다.
- **첨부 PDF 본문까지 (2026.9.5)** — '전체' 검색이 첨부 PDF 의 글자까지 봅니다. 제목·초록에 없는 낱말이 PDF 에 있으면 그 문헌이 뒤에 추가되고 행 아래 **발췌 한 줄**(일치 부분 강조)이 보입니다. 고급 검색에 **본문(PDF)** 필드가 생겼습니다. 색인은 첨부 뒤 배경에서 저절로(부팅 뒤·첨부 뒤 1분 안) 만들어지고, 상태와 [지금 색인]은 `⚙설정 › 서재 › PDF 본문 색인` 에 있습니다. 스캔본은 글자 인식 뒤에 잡힙니다. 색인은 낮은 우선순위로 돌고 상태줄에 `k/할 것` 진행이 보입니다(새로 바뀐 파일만 셉니다). 2026.9.19 부터 엔진을 바꿔 서재 802편 기준 43분 → 3.4분입니다. 2026.9.16 부터는 누르거나 입력하는 동안에는 다음 파일을 열지 않아, 색인 중에도 앱이 바로 반응합니다.

### 3.9 검색 결과에서 더 하는 일

**검색어 하이라이트 (2026.8.7)** — 제목·저자·수록처에서 검색어와 일치하는 부분을 표시합니다.

- 제목·저자·수록처 어디에도 검색어가 없으면 일치한 칸을 표시합니다(`일치: 초록·주제어` 처럼).
- 검색 결과 화면에서만 표시합니다.
...
- **PDF** — 새 서지를 만들고 그 PDF 를 첨부합니다(제목은 파일명으로 임시 → 나중에 서지정보 채우기·편집). **문헌 행 위에 놓으면 그 문헌의 첨부가 됩니다**(행에 테두리가 생기고, 파일 이름도 서지 기준으로 정리됩니다). 빈 곳에 놓는 동작은 그대로 새 서지입니다.
- **RIS·BibTeX·XML 등 서지 파일** — 임포터로 서지를 가져옵니다([3.9](#311-파일-가져오기) 와 동일 처리). 이미 있던 문헌은 합쳐집니다.
- **컬렉션(뷰 또는 사이드바 항목)에 놓으면** 그 컬렉션으로 배정되고(서지 파일도 2026.9.18 부터 — 합쳐진 문헌까지), **미분류·전체 서지에 놓으면** 미분류로 추가됩니다(휴지통은 무시).
...
- **행을 클릭하면 한글이 그 인용 자리로 이동**합니다.
- **✎(더블클릭 또는 우클릭)** 으로 그 자리에서 고칩니다 — 표기 방식·쪽·접두/접미 문구·병렬 인용의 구성 문헌.
- 병렬 인용은 **구성 문헌을 하나씩 뺄 수 있습니다**(마지막 하나는 뺄 수 없습니다).
...
1. 문서에 연결.
2. **인용 편집 창 머리의 [참고문헌 생성·갱신]**(2026.9.6 — 갱신하면 표기가 달라질 인용이 있으면 호박색) · `도구` 메뉴 · **사이드바 문서 우클릭** · 상태줄의 **'참고문헌 생성·갱신 필요'** 칩, 어디서 눌러도 같습니다(인용 삽입 때마다 내부적으로도 자동 실행). 인용 편집 창에서 갱신하면 본창의 칩도 함께 꺼지고, 본창에서 갱신하면 열려 있는 인용 편집 창의 목록이 다시 읽힙니다.
3. 순서: 인접 병합 → 문서 순서대로 재서식 → 삭제된 인용 기록 정리 → 참고문헌 재구축.
4. 상태줄에 `갱신 완료 · 인용 N · 참고문헌 M건`.

- 인용·참고문헌은 항상 **내 서지(master) 기준**으로 렌더됩니다.
- 문서에만 있던 문헌은 내 서지로 자동 승격되고, 기록이 없는 인용은 orphan 으로 분리해 보고합니다.
...
- 원본은 그대로 남습니다. 참고문헌 재생성도, 인용 양식 변경도 원본에서 계속됩니다. 사본은 제출용입니다.
...
- 인용 데이터는 파일 경로가 아니라 **서재의 `docstore/<uuid>.json`** 에 보관.
- 따라서 문서명 변경·폴더 이동·다른 PC 이어작업에 강건합니다.
- **인용한 서지가 문서 안에 함께 담깁니다 (2026.9.5)** — 누름틀의 **속성(메모) 자리**에 담겨 화면·인쇄에 전혀 나오지 않습니다. 문서만 받은 PC 에서도 인용 기록과 인용된 서지가 살아납니다(개인 메모·태그·초록·첨부 PDF 는 담지 않습니다). 큰 서재도 조각으로 나눠 담아 크기 제한이 없고, 2026.8.x 방식으로 담겨 지면에 보이던 문서는 다음 저장 때 자동으로 정리됩니다.
- ⚠ 2026.9.4 이하로 저장한 문서나 그 버전으로 받는 쪽에서는 기록이 없어 '기록이 없는 인용(orphan)'으로 보고됩니다 — 그때는 서재 폴더(또는 해당 docstore json)를 공유하거나 공용 클라우드 폴더를 씁니다.
...
| **IEEE** (2026.9.5) | 번호형 `[1]`, 7인 이상은 첫 저자 et al. 영문 투고용 |
| **Chicago 17 (저자-연도)** (2026.9.5) | `(Smith and Lee 2020, 23)` |
| **MLA 9** (2026.9.5) | 연도 없는 `(Smith and Lee 23)` |
...
**밴쿠버 부모에도 파라미터가 적용됩니다**(2026.8.1 부터).

- 저자 축약(`et al.` 기준·표시 인원) · 강조(제목/수록처) · DOI 표기 · 쪽 표기가 실제로 반영됩니다. 위 표의 밴쿠버 기본값은 **저널 스타일이 명시하지 않았을 때만** 쓰입니다.
- **쪽을 함께 인용하면 `[1](p23)`** 처럼 번호와 쪽을 구분합니다(`[1, 23]` 은 다른 문헌 번호로 읽힐 수 있습니다). 학회가 쪽 표기를 지정했으면 그 표기를 씁니다.
...
표 안의 내용만 바꾸고 경계는 한글이 지키므로 갱신이 원고 본문을 건드리지 않습니다.
...
- 위치: `<서재 폴더>/journal_styles/<id>.json` — 클라우드 서재면 PC 간 동기화, 파일 복사로 공유 가능.
- 내장 스타일은 삭제 불가. **같은 이름으로 저장하면 '사용자 판' 이 우선**하고, 사용자 판을 지우면 내장 복원.
- 부모가 **Vancouver 여도 파라미터가 반영됩니다**(2026.8.1) — 저자 축약·강조·DOI·쪽 표기가 적용됩니다([5.1](#51-내장-엔진-스타일-6종)). 번호형[n]은 참고문헌에 반영됩니다.
...
- 레일의 **'전체 서지'** 에서 전체 목록.
- **`library.json` 이 원본 데이터**(원자적 저장). SQLite `.db` 는 검색 색인용 로컬 프로젝션(재생성 가능·클라우드 밖).
- 문제 시 환경변수 `IBIS_BACKEND=file`(옛 이름 `HWPENDNOTE_BACKEND` 도 그대로 인정)로 파일 백엔드 복귀.
- 서재 폴더를 클라우드에 두면 여러 PC 공유([9.6](#96-서재-위치클라우드-폴더-동기화)).
...
- 카운트는 **하위 포함 재귀 합계**.
- 상위 컬렉션을 삭제하면 하위는 상위로 재지정되고, **문헌 자체는 서재에 남습니다.**
...
- 웹판에는 별도 태그 목록이 없습니다 — **태그를 보는 주 인터페이스는 태그 그래프 뷰**.

### 6.5 태그 그래프 뷰

태그=노드, 같은 문헌에 함께 달린 태그=연결선(자체 포스 레이아웃, 외부 서재 없음). 노드 크기는 문헌 수에 비례.
...
- **★즐겨찾기** — 문헌 우클릭 › '즐겨찾기 토글'.
- **최근 추가** — 최근 30일에 담은 문헌(2026.9.4 부터). 같은 날 안에서도 시각순으로 정렬됩니다.
...
- **폼이 유형에 맞게 바뀝니다 (2026.8.3)** — 유형을 고르면 그 유형이 실제로 쓰는 칸만 남습니다. 학위논문이면 수여기관·학위, 단행본 수록 장이면 수록서·편저자·판차·권, 법령이면 발령주체·호수·시행일 하는 식입니다. 적어 넣은 값은 모두 저장됩니다.
...
- **KOSIS 통계표 주소로 담기 (2026.9.13)** — 같은 칸에 통계표 주소(`statHtml.do?orgId=…&tblId=…`)를 넣으면 **데이터세트 서지**가 됩니다(표명·작성기관·통계조사·수록 기간·인출일). KOSIS 화면에서 찾은 표를 바로 인용할 수 있습니다.
- **앱 안 브라우저로 연 페이지 담기 (2026.9.5)** — 로그인이나 스크립트가 필요한 기사·웹 페이지는 **[브라우저로 열기]** 로 앱 안 브라우저를 띄워 직접 이동한 뒤 **[열린 페이지 담기]** 를 누릅니다. 지금 떠 있는 페이지를 그 상태 그대로 읽어 제목·매체·저자·게시일을 채웁니다(주소 조회와 같은 방식).
- **단행본은 더 채워집니다 (2026.9.1)** — 카카오 책·알라딘을 함께 보아 **책소개·역자(저자와 분리)·ISBN·분류·쪽수**를 가져옵니다. 이미 서재에 있는 단행본에도 일괄로 적용할 수 있습니다.
- **여러 권으로 된 책·연속간행물 (2026.9.3)** — 단행본이 여러 권짜리면 **'권'** 칸이, 연속간행물이면 **'권·호'** 칸이 함께 나옵니다.
- **판본이 여럿이면 직접 고릅니다 (2026.8.4)** — 같은 책의 초판·개정판·다른 출판사본이 함께 잡히면 후보를 보여 줍니다. 판차·출판사·연도를 보고 고르면 그 값으로 채웁니다.
...
- **담는 순간 이름이 정리됩니다 (2026.9.4)** — PDF 를 끌어다 놓거나 원문 확보로 붙일 때 곧바로 '첫저자 - 연도 - 제목' 으로 바뀝니다. 아래 일괄 정리는 이전에 쌓인 것을 정리할 때 씁니다.
- **첨부 이름 일괄 정리** — `도구` › '첨부 이름 일괄 정리'(이전 이름: 첨부 자동 이름 정리). 서재 전체 첨부를 서지 기반 파일명으로 한 번에 정리하고 결과(정리·유지·실패)를 브리핑합니다.
- **한 건만 — [자동 이름] (2026.9.13)** — 이름 바꾸기 창의 **[자동 이름]** 을 누르면 **첫저자 - 연도 - 제목**을 칸에 채웁니다. 바로 바뀌지 않으니 보고 손볼 수 있습니다. 첨부 우클릭의 '자동 이름으로 바꾸기'도 다시 쓸 수 있습니다(국문 이름이 성만 남던 것도 고쳤습니다).
...
- 자동으로 동기화됩니다. 서지를 고치면 1분 안에 올라가고, 다른 창을 보다 Ibis 로 돌아올 때 한 번, 그리고 10분마다 전량을 맞춥니다. 사이드바 공유 배지가 동기화 중이면 돕니다.
- 우클릭 **[참가자·초대 관리…]** 로 누가 들어와 있는지 보고 잘못 나간 초대 링크를 끊습니다. 같은 창에서 초대장도 만듭니다. **[활동 기록…]** 은 누가 언제 무엇을 바꿨는지 보여 줍니다.
...
- 문헌·컬렉션 구성은 물론 **개인 메모·즐겨찾기·초록까지** 갑니다. 받는 쪽이 본인이기 때문입니다. **PDF 원문 파일은 여기서도 올라가지 않습니다.**
- **다른 PC 에서 이미 켰다면 그것을 먼저 받아 합칩니다.**
...
- **PDF 파일까지 여러 PC 에서 쓰려면** 지금처럼 서재 폴더를 OneDrive 같은 클라우드 폴더에 두세요 — 서재 동기화는 그것을 대체하지 않고, 폴더 동기화가 약한 부분(같은 서지를 양쪽에서 고쳤을 때 생기는 충돌본)을 서버가 칸 단위로 정리합니다. **둘을 함께 쓰는 것이 권장 구성입니다.**
- **컬렉션의 모양도 따라옵니다**(2026.9.19) — 이름·부모·색·순서·숨김을 바꾼 것, 스마트 컬렉션, 기본 인용 스타일과 서재 옵션(참고문헌 제목 등)이 다른 PC 에 반영됩니다. 접힘 상태·휴지통·PDF 본문 색인은 PC 마다 따로입니다.
...
- 하이라이트가 문장으로 전달되므로, 받는 쪽이 원문을 확보해 열면 판본이 달라도 문장으로 자리를 찾아 붙습니다.
...
2. **컬렉션의 이름·색·정렬 순서를 바꾼 것은 전파되지 않습니다.** 참가자마다 달라지는 것을 막기 위해 제외했습니다. 문헌을 넣고 빼는 것은 정상으로 오갑니다.

## 7. 원문 확보·PDF 뷰어

> **준법 정책** — 구독 학술 DB(DBpia·KISS·교보스콜라·ScienceDirect·Springer 등)에 자동으로 접속하지 않습니다. 자동 로그인·클릭·크롤링·다운로드를 하지 않고, 도서관 교외접속으로 **브라우저에 열어주기만** 합니다. 무료 공개(OA)·무료 기관 리포지터리만 자동 확보합니다.
...
1. `⚙설정` 의 **'도서관 경유 URL'** — **목록에서 소속 기관을 고릅니다**(2026.9.2 부터 34개 기관). 목록에 없으면 주소를 직접 넣습니다(예: `https://proxy.○○.ac.kr/_Lib_Proxy_Url/`). 옆의 **[확인]** 으로 그 주소가 실제로 응답하는지 눌러 볼 수 있습니다.
...
- 프록시는 로그인하지 않은 접근을 거부합니다. ⚙설정의 **[로그인 열기]** 로 먼저 로그인해 세션을 만드세요([9.4](#94-도서관-교외접속프록시-경유-설정)).
...
- **글자가 안 보이는 쪽은 그 쪽만 다시 그립니다 (2026.9.15)** — 뷰어가 그리지 못하는 글꼴 조합이 원인입니다. 글자가 빠진 쪽을 감지해 그 쪽만 Windows 내장 PDF 엔진으로 다시 그립니다(따로 설치할 것은 없습니다). 글자 선택·찾기·주석은 그대로입니다.
- **글자가 어긋나게 추출되던 PDF (2026.9.15)** — 화면은 정상인데 검색·AI 가 읽는 글자가 어긋나는 문서가 있습니다(`Korean Journal of Sociology` 가 `,PSFBO+PVSOBM` 처럼). 되돌릴 수 있는 것은 되돌리고, 되돌릴 수 없는 쪽은 그 쪽만 다시 읽습니다(요약·PDF 대화·쪽 찾기에서). 정상인 문서는 바뀌지 않습니다.
...
- 저장: PDF 옆 `<파일명>.annots.json` 사이드카 → 서재 동기화를 따라감.
- 사이드바 **'주석 검색'** 으로 서재 전체 주석 검색 — 보관한 AI 문답([7.8](#78-pdf-창-대화-탭))도 함께 나오고 결과줄에 `주석 N · AI 대화 M` 으로 갈라 셉니다. **주석 개수 배지와 인쇄 모음은 주석만** 세므로 보관 답변으로 부풀지 않습니다.
...
- 답변이 생성되는 대로 표시됩니다(스트리밍 — Claude CLI·Antigravity CLI). [중지] 하면 받은 데까지 남습니다.
- 표시 세 가지:
  ...
  | (쪽 미확인) | 따옴표 친 인용인데 원문에서 못 찾았습니다. 인용하기 전에 확인하세요 |

  쪽 번호는 모델이 적은 것을 믿지 않고 **앱이 원문에서 되찾아 확정합니다.** 쪽을 걸친 인용(`p.3-4`)도 확인해 링크합니다.
- 남길 답은 **[보관]** 하면 상단에 접혀 쌓입니다. `새 대화` 는 새 대화를 시작하며, 지난 대화는 남습니다(아래).
...
첨부 PDF 본문을 읽어 한국어 요약 + 태그 + 키워드를 생성합니다.

**무엇을 뽑을지는 서지 유형에 따라 다릅니다 (2026.9.11)** — 여섯 갈래로 나뉩니다.

| 유형 | 뽑는 것 |
|---|---|
| 실증 논문 | 연구질문·방법·핵심결과·시사점/한계 |
| 저술·논설 | 핵심 주장·논증 구조·반론 |
| 정책·연구 보고서 | 정책 문제·근거 자료·제안·이해관계 |
| 법령·법률안 | 목적·적용 대상·주요 조문·부칙 — 근거는 **조문 번호**로 |
| 판례·해석례 | 사실관계·쟁점·판단·법리 (비판적 검토는 평석 관점) |
| 회의록 | 안건·발언·쟁점·결정 |

상세 패널의 AI 버튼에 마우스를 올리면 **그 문헌에서 무엇을 뽑을지** 미리 말합니다('AI 법령 노트 — 목적·적용 대상·주요 조문·개정 연혁'). 태그를 뽑는 면도 유형마다 다릅니다(법령은 규율 주제·대상·기관).
...
- **스캔본도 읽습니다 (2026.9.2)** — 글자가 없는 PDF 는 Windows 내장 글자 인식으로 본문을 뽑아 넘깁니다.
  - 처음 한 번은 시간이 걸립니다(수십 쪽이면 십수 초~수 분). 자동으로 실행됩니다. 2026.9.16 부터 **시작하기 전에 먼저 알립니다** — 「스캔본입니다 — 글자를 먼저 읽습니다(42쪽). 긴 문서는 몇 분 걸릴 수 있습니다」(서지 채우기는 앞뒤 몇 쪽만 읽습니다). 그 뒤 진행이 **「스캔본이라 글자를 먼저 읽습니다 — 텍스트 인식 3/42쪽」** 으로 상태줄·진행 상자에 보입니다.
  - 한 번 읽어 두면 그 PDF 는 다음부터 바로 되고, 그때는 안내가 뜨지 않습니다.
  - AI 가 본문을 필요로 할 때만 실행됩니다(요약·비판적 검토·PDF 대화·서지 채우기). 뷰어로 여는 것만으로는 실행되지 않습니다.
...
- 여러 편을 골라 일괄 요약을 실행하면 `⚙설정 › AI` 의 **[동시 실행]**(1~5, 기본 3)만큼 동시에 처리합니다.
...
DOI 가 있으면 Crossref·KCI 를 먼저 조회하고, 빈 항목이 남을 때만 PDF 표지를 AI 로 분석합니다.
...
- 기존 값은 덮어쓰지 않고 **빈 곳만 보완**(DOI 조회 값 우선).
...
두 자리는 같은 모양이고, '동시 실행'·'CLI 상태'는 맨 아래 «공통 · 1차·폴백 모두에 적용» 에 있습니다.
...
| Ibis AI | 크레딧(키·CLI 불필요) | **구글 로그인**이 필요합니다(⚙설정 › 클라우드). 시작하면 **체험 크레딧 100**, [베타 무료 구독]을 시작하면 **하루 50크레딧**이 매일 채워집니다(아래 참고). 남은 양은 화면에 표시됩니다. ⚠ 제공자 목록에는 아직 `(준비 중 — 정식 릴리스 때 활성화)` 라고 적혀 있습니다 — 베타 기간의 표기입니다 |

- **Ibis AI 크레딧 (2026.9.2)** — 글자 수(토큰) 기준 종량제라 짧은 요청은 적게, 수백 쪽 PDF 는 많이 듭니다. 실행 전에 예상 크레딧을 보여 주고 동의를 받으며, 남은 크레딧이 모자라면 그 자리에서 알립니다.
- **체험 100 → 베타 무료 구독 (2026.9.14)** — 처음 시작하면 체험 크레딧 100 을 받습니다. 다 쓰면 채워지지 않습니다. 이어서 쓰려면 [베타 무료 구독] 을 시작하면 그 자리에서 하루 50크레딧이 매일 채워집니다. 승인 대기는 없습니다.
  - 베타 기간에는 요금이 없고 결제 수단도 받지 않습니다. 정식 요금이 정해지면 구독자에게 먼저 알리고, 그때 계속할지 선택합니다.
  - **이미 [베타 무료 구독]을 시작하신 계정은 그대로 하루 50크레딧입니다** — 판올림해도 바뀌지 않고 안내도 뜨지 않습니다.
  - 구독한 적 없는 계정은 판올림할 때 체험으로 옮겨지고 그때 안내가 한 번 뜹니다. 거기서 구독을 시작하면 다시 하루 50입니다.
  - 계정을 바꾸면 플랜과 남은 크레딧이 **즉시** 반영됩니다(2026.9.14).
- **Ibis AI 가 바쁠 때는 기다립니다 (2026.9.5)** — 여러 사람이 동시에 써서 서버 동시 한도에 걸리면 'AI 제공자 오류(429)' 대신 여유가 생길 때까지 최대 90초 동안 다시 시도합니다. 기다리는 동안 상태줄에 표시되고, 끝내 처리되지 않으면 그 사실을 알린 뒤 폴백 제공자가 있으면 그쪽으로 넘어갑니다. **'동시 실행'은 Ibis AI 에서 2건까지만** 적용됩니다.
- **Ibis AI 를 1차로 두면 모델·추론 강도 칸이 없습니다** — 작업별로 Ibis 가 정합니다(요약·검토·대화 gpt-oss-120b · 관련도 채점 bge-reranker-v2-m3). 그 자리에 그렇게 적혀 있습니다(2026.9.5).
...
`⚙설정`(톱니)은 **일반 · AI · 검색·원문 · 클라우드 · 서재** 5개 탭입니다. 레일 맨 아래 ⚙ 로 엽니다.
...
| **검색·원문** | 이메일·선택 키([9.1](#91-api-키이메일-발급)) · 도서관 원문([9.4](#94-도서관-교외접속프록시-경유-설정)) |
| **클라우드** | 구글 로그인 · 공유 중인 컬렉션 목록 · 서버 사용량([6.10](#610-컬렉션-공유)) |
| **서재** | 서재 위치·백업·업데이트 확인([9.6](#96-서재-위치클라우드-폴더-동기화)) |

> **도구 패널은 없앴습니다.** 그 안에 있던 것은 이렇게 갈라졌습니다 — 가져오기·내보내기·컬렉션 공유는 `파일` 메뉴, 일괄 정리(중복·원문·AI 태그·첨부 이름)는 `도구` 메뉴, DOI·ISBN 추가는 `새 문헌 추가` 창의 **식별자로 채우기**, 서재 위치·백업·업데이트 확인과 클라우드 로그인은 `⚙설정` 입니다.

설정 칸은 대부분 **'빈 칸은 유지' 규칙**이라 비우고 저장해도 지워지지 않습니다(삭제는 '지움' 버튼).

### 9.1 API 키·이메일 발급

*(⚙설정 › 검색·원문 탭. **학술 DB 와 법·정책은 모두 앱 내장 키로 동작해 입력 칸이 없습니다**(2026.9.9) — 아래가 남은 전부이고, 셋 다 **선택**입니다.)*

![설정 — 검색·원문 탭(이메일·선택 키·도서관 경유)](images/settings-api.png)

| 항목 | 발급처 · 절차 | 입력 칸 | 문의 |
|---|---|---|---|
| **Semantic Scholar** | 발급 선택 — `semanticscholar.org/product/api` (무료, 며칠 걸림) | `Semantic Scholar` | — |
...
| **도서관 원문 연결** | 발급 아님. 기관 프록시 주소([9.4](#94-도서관-교외접속프록시-경유-설정)) | `도서관 경유 URL` + `로그인 열기` — **칸 하나입니다**(2026.9.13 — '도서관 홈페이지' 칸과 '항상 도서관 경유' 체크란을 뺐습니다) | — |

- **Semantic Scholar** — 내장 키로 동작하지만 내장 키의 한도(초당 1회)는 모든 사용자가 나눠 씁니다. 자주 쓰면 개인 키를 넣으세요.
- **이메일 칸은 예외** — 비우고 저장하면 삭제됩니다. Unpaywall·Crossref 두 값에 같은 주소가 저장됩니다.
- **Google Books** — 단행본 **ISBN 조회** 보조용(선택, [3.4](#35-isbn으로-단행본-추가)). 키 없이 쓰면 공용 쿼터 초과가 흔해 보조로만 씁니다. ISBN 조회 자체는 내장 키로 국립중앙도서관을 씁니다.
- **키를 하나도 넣지 않아도** 학술 DB 9종과 법·정책 5갈래가 전부 동작합니다. 위 세 칸은 선택 사항입니다.

### 9.2 저장된 키 삭제 — '지움'

1. 설정된 키는 `● 설정됨` 으로 표시되고 옆에 **'지움'** 버튼이 나타납니다.
2. '지움' → **'저장'** → 실제 삭제됩니다. 지워도 검색은 그대로입니다 — 위 세 칸은 전부 선택이고, 학술 DB·법·정책은 내장 키로 동작합니다.
...
1. **'도서관 경유 URL'** — 기관 프록시 주소(예: `https://proxy.knue.ac.kr/_Lib_Proxy_Url/`). 목록에서 고르거나 직접 넣습니다.
2. **'로그인 열기'** 로 브라우저에서 먼저 한 번 로그인해 세션을 만듭니다(그 주소의 로그인 페이지가 열립니다).

**2026.9.13 부터 칸이 하나입니다** — '도서관 홈페이지' 칸과 '원문 열기 시 항상 도서관 경유' 체크란을 뺐습니다. 경유 URL 하나가 로그인 주소와 경유 여부를 함께 정합니다: 넣으면 원문 열기·확보가 도서관을 거치고(법제처·국회도서관·KCI 같은 무료 사이트는 바로), 비워 두면 늘 바로 엽니다.

- 이전에 경유 URL 만 넣고 체크란은 켜지 않았던 경우, 이제 경유로 열립니다. 교내 고정 IP 처럼 경유가 오히려 방해되는 드문 경우는 환경변수 `LIBRARY_PROXY_AUTO=0` 으로 끕니다(화면에는 없습니다).
...
- 로그인하지 않으면 프록시가 접근을 거부하므로 먼저 로그인해야 합니다.
...
| 항목 | 기본 위치 | 클라우드 동기화 |
|---|---|---|
| 서재(서지) | `%LOCALAPPDATA%\Ibis\library.json` | 폴더를 클라우드로 지정 시 O |
| 첨부 PDF | 서재 폴더의 `attachments\` | 서재를 따라감 |
| docstore(문서별 인용) | 서재 폴더의 `docstore\` | 서재를 따라감 |
| 저널 스타일 | 서재 폴더의 `journal_styles\` | 서재를 따라감 |
| 받은 글꼴(2026.9.4 까지) | `%LOCALAPPDATA%\Ibis\fonts\` | X (PC 로컬) — 2026.9.5 부터 쓰이지 않음 |
| 비밀정보(API 키·테마·AI) | `%LOCALAPPDATA%\Ibis\secrets.json` | **X (항상 로컬)** |

- 옛 폴더(`%LOCALAPPDATA%\hwp-endnote`)를 쓰던 사용자는 **첫 실행 때 파일 단위로 자동 이관**되므로 할 일이 없습니다.
- 비밀정보는 권한 `0o600` 으로 저장되며 항상 로컬 — **클라우드 서재를 써도 API 키·테마·글꼴·AI 설정은 PC 마다 따로** 넣어야 합니다.
- **환경변수가 `secrets.json` 보다 우선**합니다(`SCIENCEON_*`·`UNPAYWALL_EMAIL`·`LIBRARY_PROXY_PREFIX/AUTO`·`LIBRARY_LOGIN_URL`·`IBIS_LIBRARY` 등).
- 서재 경로 우선순위: env `IBIS_LIBRARY` > secrets `library_path` > 기본.
- 옛 환경변수 이름 `HWPENDNOTE_LIBRARY`·`HWPENDNOTE_BACKEND` **도 그대로 인정**됩니다(새 이름 `IBIS_LIBRARY`·`IBIS_BACKEND` 가 우선).
- `⚙설정 › 서재 › '탐색기에서 보기'` 로 확인.

### 9.6 서재 위치·클라우드 폴더 동기화

`⚙설정 › 서재` 탭입니다(`파일` › '설정 › 서재…' 로도 바로 열립니다).

| 버튼 | 동작 |
|---|---|
| **이 서재 폴더 변경** | 현재 데이터(+첨부·docstore·저널 스타일)를 지정 폴더로 이전. 대상에 `library.json` 이 있으면 그것을 **채택**, 없으면 **복사·이전** |
| **새로 만들기** | 빈 폴더에 새 서재 생성 후 전환(이미 있으면 오류). 2026.9.6 부터 **확인문**이 먼저 뜹니다 — 지금 서재의 서지 건수, 첨부가 따라오지 않는다는 것, 다른 서재를 이어 쓰려면 [열기]라는 것, 내 서재 동기화가 켜져 있을 때의 결과 |
| **열기** | 다른 폴더의 기존 서재에 **복사 없이 연결·전환**(없으면 오류) |
| **사본 저장(백업)** | 다른 폴더로 **복사만**(전환 안 함). 대상에 이미 있거나 현재와 같으면 오류 |

- '새로 만들기'·'열기' 는 전환 전에 현재 서재를 자동 저장합니다.
- 서재가 바뀔 때마다 `%LOCALAPPDATA%\Ibis\library_switch.log` 에 한 줄(언제·어디서·어디로)이 남고 직전 위치를 기억합니다(2026.9.6). 어느 날 첨부가 전부 안 보이면 먼저 이 탭의 **서재 위치**가 쓰던 폴더인지 보세요 — 다른 곳이면 [열기]로 그 폴더를 고르면 됩니다.
- **'이 PC 의 서재' 목록 (2026.9.16)** — 이 PC 에서 써 본 서재가 **둘 이상일 때** 이 탭에 목록이 생깁니다(하나뿐이면 안 보입니다). 줄마다 서지 건수가 적혀 있어 어느 쪽이 쓰던 서재인지 구분할 수 있고, [열기]로 바꿉니다.
- **빈 서재에서 내 서재 동기화를 켜면 먼저 묻습니다 (2026.9.16)** — 서버 사본이 내려와 채워지지만 **첨부 PDF 는 따라오지 않습니다**. 원래 쓰던 서재가 이 PC 에 있으면 **[그 서재 열기]** 와 **[여기서 켜기]** 중에 고를 수 있습니다.
...
![설정 — 표시(테마)](images/settings-display.png)
...
| | |
|---|---|
| ![Ivory](images/theme-ivory.png) **Ivory — 기본**(종이톤) | ![Slate](images/theme-slate.png) Slate |
...
**글꼴** — 2026.9.5 부터 화면 글꼴은 **Pretendard 하나**입니다. 글꼴 선택과 내려받기(본명조·나눔·JetBrains Mono)를 뺐고, 종이톤 테마 둘도 Pretendard 로 맞췄습니다. Pretendard 를 못 쓰는 환경에서는 맑은 고딕으로 대체됩니다. 예전에 받아 둔 글꼴 파일(`%LOCALAPPDATA%\Ibis\fonts`)은 더 쓰이지 않으니 지워도 됩니다.
...
- 테마·항상 위 설정은 PC 로컬(`secrets.json`)이라 클라우드로 공유되지 않습니다.

### 9.8 온보딩을 건너뛰었다면

- 모든 항목(서재 위치·이메일·AI, 그리고 온보딩에 없는 도서관 경유)은 `⚙설정` 에서 언제든 변경 가능.
...
- **API 키 없이 쓸 수 있나요?** — 예. 학술 DB 9종과 법·정책 5갈래 모두 내장 키로 동작하고, 인용 삽입·서지 관리·스타일도 모두 됩니다.
- **여러 PC 에서 같은 서지를?** — 서재 폴더를 클라우드로 지정하고 다른 PC 에서 '열기'. 단 API 키·테마·글꼴·AI 설정은 PC 마다 따로.
...
- **읽는 논문에 대해 AI 에게 물어볼 수 있나요?** — 예. PDF 창의 **'대화' 탭**이 그 논문 한 편만을 근거로 답하고, 답 속 `(p.쪽)` 을 누르면 근거 자리로 갑니다. 지난 대화는 그 PDF 에 남고, 남길 답은 [보관] 할 수 있습니다([7.8](#78-pdf-창-대화-탭)).
