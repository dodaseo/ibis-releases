...
- **글자가 안 보이는 쪽이 있으면 그 쪽만 다시 그립니다 (2026.9.15)** — 제목 상자가 테두리만 그려지고 글자가 없던 문제입니다. 문서 잘못이 아니라 뷰어가 그 조합을 못 그리는 것이라 판을 올려도 안 풀렸습니다. 이제 **잉크가 가지 않은 쪽**을 알아채 그 쪽만 Windows 내장 PDF 엔진으로 다시 그립니다. 글자 선택·찾기·주석은 그대로입니다.
