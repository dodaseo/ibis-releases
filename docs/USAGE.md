...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.8**.
...
2. 찾기 줄 아래 **체크 행**에서 어디서 찾을지 고릅니다: `전체` │ **국내**(KCI·ScienceON·국립중앙도서관) │ **국외**(OpenAlex·Crossref·ERIC·arXiv·PubMed). 행 오른쪽 끝에 **예상 크레딧**(`+2~7` 처럼)이 적힙니다.
...
>
> 두 찾기는 **같은 자리(찾기 줄 아래)에서 같은 방식**으로 DB 를 고릅니다 — `전체 │ 국내 … │ 국외 …`, '국내'·'국외' 글자를 누르면 묶음이 통째로 켜지고 꺼지며 조합은 자유입니다. 다만 **목록과 기억은 다릅니다**: 키워드로 찾기는 9개(S2 포함)를 `localStorage` 에 기억하고 기본 꺼짐이 arXiv·PubMed·S2, 질문으로 찾기는 8개(**S2 없음** — 질의 라우팅이 그 DB 를 쓰지 않습니다)에 기억은 그 판 한정, 기본 꺼짐이 arXiv·PubMed 입니다. '내 서재' 칸은 양쪽 다 없습니다.

### 3.2 검색 소스

레일 **두 번째 아이콘 [키워드로 찾기]** 입니다. 어디서 찾을지는 **찾기 줄 바로 아래 체크 행**에서 고릅니다 — `전체 │ 국내 KCI·ScienceON·국중도 │ 국외 OpenAlex·Crossref·ERIC·arXiv·PubMed·S2`(arXiv·PubMed·S2 는 주제 범위가 좁아 **기본 꺼짐**). **'국내'·'국외' 글자를 누르면 그 묶음이 통째로 켜지고 꺼지며**, 원하는 대로 섞어도 됩니다(KCI+ERIC 만 같은 조합도 가능). 고른 조합은 기억되고, 행 오른쪽 끝에 `○○에서 찾기` 라고 지금 조합이 적힙니다(고른 것이 없으면 'DB 를 하나 이상 고르세요').

소스는 **국내 3종 · 해외 6종** 이며, 둘 이상을 켜면 병렬로 질의한 뒤 중복을 합칩니다.
...
| — | 내 서지 | 보관한 문헌 — 체크 항목이 아닙니다(2026.9.7~9.8 에 두 찾기 모두에서 빠졌습니다). 내 문헌은 **서재 모드의 찾기 줄**에서 찾고([3.8](#38-내-서지-전문검색fts)), 찾은 결과가 이미 서재에 있으면 행에 **'보관됨'** 배지가 붙습니다 | — |
...
- **유형(공통)** · 정렬 · 발행년 범위. (어느 DB 에서 찾을지는 고급 검색 안이 아니라 **검색줄 아래 체크 행**입니다.)
...
- 모든 항목(서재 위치·이메일·AI, 그리고 온보딩에 없는 ScienceON·도서관 경유)은 `⚙설정` 에서 언제든 변경 가능.
