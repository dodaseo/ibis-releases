...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.16**.
...
| **레일**(맨 왼쪽) | 서재 · **찾기(학술)** · **법·정책**(저울) · **통계**(막대) · 그래프 뷰 전환(2026.9.12 — 찾기 셋은 *어디서* 찾을지고, 낱말/문장은 찾기 줄의 **키워드 \| 질문** 칩). 찾기 모드가 **같은 서재 사이드바**를 쓰고(어느 모드에서든 컬렉션을 누르면 그 뷰로 갑니다), 찾기 줄과 그 아래 DB 체크 행도 같은 자리입니다(2026.9.8). 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래). 레일의 참고문헌 갱신(⟳) 아이콘은 2026.9.6 에 뺐습니다 — 새로고침으로 오해돼서요. 갱신은 [4.5](#45-전체-갱신--참고문헌-갱신) 의 자리에서 합니다 |
...
낱말이 아니라 **연구 질의를 문장으로** 던지면, 국내외 학술 DB 를 뒤져 **관련도 순위**로 돌려줍니다. 레일 **찾기(학술)** 아이콘을 누르고 찾기 줄 왼쪽의 **[질문]** 칩을 켭니다(2026.9.12 — 별도 아이콘이 아니라 키워드/질문 **모드 칩**입니다. 통계 아이콘에서도 됩니다 — [3.2-1](#32-1-통계kosis--표-찾기--수치-보기--데이터세트로-인용-2026912); 법·정책은 키워드뿐).

결과는 **키워드로 찾기와 같은 표**에 나옵니다(2026.9.8 — 그전에는 별도 카드 목록이었습니다). 맨 왼쪽에 **관련도 칸**(순위 · 막대 · 0~1 점수)이 하나 더 붙고, 그 칸은 질문으로 찾기에서만 보입니다. 최근 질의와 사정 고지(잘린 것·빠진 것·실패)는 표 바로 위 한 줄에 있습니다. 질의에서 읽어 적용한 조건도 여기 **조건 …** 으로 적힙니다.
...
2. 찾기 줄 아래 **체크 행**에서 어디서 찾을지 고릅니다: `전체` │ **내 서재** │ **국내**(KCI·ScienceON·국립중앙도서관) │ **국외**(OpenAlex·Crossref·ERIC·arXiv·PubMed·S2). 행 오른쪽 끝에 지금 조합(`KCI·ScienceON·국중도 외 3에서 찾기`)과 **예상 크레딧**(`+2~7` 처럼)이 적힙니다. **키워드로 찾기와 같은 목록·같은 기억**입니다(2026.9.12 — 한쪽에서 끄면 다른 쪽도 꺼져 있습니다).
   ...
   - **법·정책 다섯 갈래는 여기에 없습니다**(2026.9.11) — [3.2](#32-검색-소스) 의 키워드로 찾기에서만 씁니다. 법률안의 제안이유는 수백 자인데 회의록은 안건 한 줄이라 채점 재료의 길이가 달라 법률안이 상위를 독식했습니다. 그 문제를 풀기 전까지 뺐습니다.
3. 진행이 단계로 보입니다 — `검색어 만들기 → DB에서 찾기 → 관련도 매기기 → 정리`(지금 단계가 점멸).
4. 최근 질의는 셋만 보이고 나머지는 **[+N ▾]** 에 접혀 있습니다 — 누르면 **크레딧 0** 으로 그때 결과가 되살아납니다.

- **찾는 일은 AI 가 합니다** — 문장을 그대로 받아 DB 를 뒤지고 관련도를 매깁니다. 낱말을 직접 고르지 않아도 됩니다(진행 단계의 '검색어 만들기'가 그 대목입니다).
...
- **저자·학술지만 묻는 질문에는 관련도가 없습니다 (2026.9.13)** — "양연동의 연구물을 찾아줘" 처럼 주제 없이 사람이나 학술지만 물으면 **저자 검색**으로 가고, 관련도 칸 대신 **최신순**으로 섭니다. 조건이 이미 정답을 보장하기 때문이고, 채점을 건너뛰니 **크레딧도 들지 않습니다**. 종전에는 열 건이 다 맞는데도 관련도가 낮게 매겨져 일부가 흐려 보였습니다 — 채점은 *주제가 닿는 정도*를 재는데 "○○의 연구물"과 그 사람 논문은 주제가 닿지 않기 때문입니다.
...
- **사이드바를 누르면 질문 화면을 닫고 그 뷰로 갑니다 (2026.9.6)** — 키워드로 찾기와 같은 동작입니다. 결과는 남아 있어 레일의 [질문으로 찾기]로 돌아오면 그대로이고, 결과 행을 컬렉션에 끌어 넣는 것도 그대로입니다. Ibis AI 를 켠 직후에도 '⚙설정에서 켜 주세요'가 뜨던 문제도 함께 고쳤습니다.
...
> 낱말로 좁혀 찾는 것은 **키워드로 찾기**([3.2](#32-검색-소스))입니다. 질문으로 찾기는 *무엇을 찾아야 할지부터 넓게 훑을 때* 씁니다. 둘은 짝입니다 — **낱말로 찾으면 키워드로 찾기, 문장으로 물으면 질문으로 찾기**.
>
> 두 찾기는 **같은 자리(찾기 줄 아래)에서 같은 방식**으로 DB 를 고르고, 2026.9.12 부터 **목록도 기억도 같습니다**(내 서재 칸·S2 둘 다 양쪽에). 법·정책은 별도 아이콘(레일 셋째)이고 키워드로만 찾습니다. 물음표로 끝나는 낱말 검색은 질문 모드를 **권하기만** 합니다 — 자동으로 바꾸지 않습니다(크레딧이 듭니다).
...
> **법·정책을 켜면 학술 DB 는 함께 찾지 않습니다**(2026.9.9) — 법령 몇 건 사이에 논문 수백 건이 끼면 나눠 찾는 뜻이 없어지기 때문입니다. 학술을 켜면 법·정책이 꺼지고, 그 반대도 같습니다.
> **법·정책은 칸 하나가 한 인용 갈래**입니다(기관 이름이 아니라 인용할 때 부르는 이름). 담으면 바로 참고문헌 서식이 나옵니다 — `교육부 (2026). 초ㆍ중등교육법. 법률, 제21659호(시행 2026. 8. 20.).`
> **관련도순만 고를 수 있습니다** — 두 API 다 정렬 인자를 받지 않아, 다른 축을 내놓으면 '정렬했는데 안 바뀐다' 가 됩니다.
...
- **모든 학술 DB 와 법·정책이 앱 내장 키로 동작합니다**(2026.9.9) — **발급받을 것이 하나도 없습니다.** ScienceON 인증키 세 값을 넣던 칸도 없앴습니다. 오래 적혀 있던 "인증키는 발급받은 공인 IP 에 묶인다" 는 실측으로 반증됐습니다(등록 IP 가 아닌 두 망에서 정상 동작).
- **DB 가 실패하면 화면이 말합니다**(2026.9.9) — 체크 행의 그 칸에 **⚠** 가 붙고(툴팁에 사유), 안내줄에 "이 DB 의 결과는 빠져 있습니다. 0건이어도 자료가 없다는 뜻이 아닙니다" 가 **남습니다**. 종전에는 상태줄에 한 번 스치고 다음 동작에 덮여, 한 소스가 죽어도 나머지 행이 그려지니 화면은 정상으로 보였습니다. 경고는 그 DB 를 다시 불러 성공했을 때만 지워지고, 체크를 자동으로 끄지는 않습니다.
...
- 찾기: **체크 행에서 DB 고르기 → 낱말 입력 → Enter** → 페이지당 20건. 체크를 바꾸면 목록이 새 조합으로 바뀌고, 실제 검색은 Enter 로 합니다(질문으로 찾기와 같은 결).
...
- **목록이 통계조사별로 묶입니다 (2026.9.13)** — 머리줄에 `조사명 · 표 수 · 작성기관`이 서고, 표마다 둘째 줄에 **수록 기간과 분류 경로**가 붙습니다. 한 조사가 표 스무 개를 내놓을 때 그중 무엇을 봐야 하는지 가르는 자리입니다. 정렬을 바꿔 조사가 흩어지면 머리줄은 사라집니다.
- **질문** — "초등학생 1인당 월평균 사교육비는 얼마나 되나?" 처럼 문장으로 물으면 표를 모아 **관련도 순**으로 돌려줍니다. 표 이름만 보고는 어느 표에 그 숫자가 있는지 알기 어려워서, 상위 5개 표는 **최근 시점 수치**가 행에 📊 발췌로 미리 보입니다(열기 전에 숫자를 봅니다). 크레딧은 학술 질문과 같습니다.
- **상세**(서지 상세와 다른 화면) — 표명·작성기관·조사·수록 기간 → **[수치 보기]** → 분류 경로·표 주석 → 인용 → 서지 정보. 읽는 화면이라 피벗·차트는 [KOSIS 에서 보기]로, 원자료는 **[CSV 저장]**(분류 코드·이름·항목·단위·시점·값)으로 받습니다. 한 번에 4만 셀까지(KOSIS 제한) — 넘으면 시점 수나 항목을 좁히라고 알립니다.
- **수치 표는 분류·항목이 행, 시점이 열입니다 (2026.9.13)** — 오래된 시점부터 오른쪽으로 가고 첫 열은 고정돼 스크롤해도 이름이 남습니다. 종전에는 분류 조합이 열이 되어 **80열에서 잘렸습니다**(사교육비 총액 155조합·시군구 출산율 2,072조합). 기본 항목은 첫 항목(대개 '전체')이고 '항목 전체'는 조작 칸에서 고릅니다.
...
- **담기·인용** — 담는 순간 **인출일**이 오늘로 적힙니다(수치는 갱신되므로 언제 봤는지가 서지의 일부). 인용은 `국가데이터처 (2025). 학교급별 학생 1인당 월평균 사교육비 [데이터세트]. 초중고사교육비조사. 2007~2025. KOSIS 국가통계포털. URL (인출일: 2026. 9. 9.)` 꼴이고, 영문 APA·Vancouver·IEEE·Chicago·MLA 도 됩니다. 데이터세트에는 PDF 확보·AI 노트·검토 버튼이 없습니다(수치는 문헌이 아닙니다).

### 3.3 여러 DB 를 한 번에(병렬 + 중복 병합)

- 체크 행에서 **둘 이상 켜면** 켠 DB 를 **동시 질의**합니다. **국내**(KCI·ScienceON·국중도)나 **국외**를 통째로 켜는 것이 흔한 쓰임이지만, 조합은 자유입니다(2026.9.7 — 그전에는 '국내 통합'·'해외 통합' 두 항목만 있었습니다).
...
- 전용 필드를 고르면 **위 체크 행이 그 DB 로 바뀝니다**(2026.9.7 — 그전에는 고급 검색 안의 소스 드롭다운이었습니다). 그 DB 가 못 하는 조건은 조용히 무시하지 않고 이유를 안내줄에 적습니다.
...
- **고급 검색은 범위를 따릅니다**(2026.9.12) — 법·정책과 통계는 두 API 다 검색어 하나만 받으므로 조건 줄이 하나(필드 '전체' 고정)이고 정렬 줄은 없습니다(관련도순뿐). 통계는 유형 줄 대신 **작성기관·통계조사** 칸(받은 목록 안에서 거름)과 **수록 기간(겹침)**, 법·정책은 **연도(시행·선고·제안·회의)** 를 씁니다(2026.9.13 — 칸 이름을 판정과 맞췄습니다. 통계의 연도는 수록 기간과 **겹치면** 걸리므로 `2015~2020` 이 1992~2026 수록 표를 떨어뜨리지 않습니다. 법·정책의 '유형' 줄은 체크 행 다섯 칸과 중복이라 숨겼습니다).
...
- **첨부 PDF 본문까지 (2026.9.5)** — '전체' 검색이 첨부 PDF 의 글자까지 봅니다. 제목·초록에 없는 낱말을 PDF 가 품고 있으면 그 문헌이 뒤에 덧붙고 행 아래 **발췌 한 줄**(일치 부분 강조)이 보입니다. 고급 검색에 **본문(PDF)** 필드가 생겼습니다. 색인은 첨부 뒤 배경에서 저절로(부팅 뒤·첨부 뒤 1분 안) 만들어지고, 상태와 [지금 색인]은 `⚙설정 › 서재 › PDF 본문 색인` 에 있습니다. 스캔본은 글자 인식 뒤에 잡힙니다. 색인은 **낮은 우선순위로 파일 사이에 쉬며** 돌고, 상태줄에 `n/전체` 진행이 보입니다(2026.9.6 — 첫 색인 수백 편이 도는 동안 창이 굼뜨던 문제). **2026.9.16 부터는 손을 대고 있는 동안 아예 물러납니다** — 누르거나 입력하는 중에는 다음 파일을 열지 않습니다. 색인이 도는 중에도 앱이 곧바로 반응합니다.
...
- **PDF** — 새 서지를 만들고 그 PDF 를 첨부합니다(제목은 파일명으로 임시 → 나중에 서지정보 채우기·편집). **문헌 행 위에 놓으면 그 문헌의 첨부가 됩니다**(2026.9.5 부터 — 행에 테두리가 생기고, 파일 이름도 서지 기준으로 정리됩니다. 종전에는 어느 행에 놓아도 새 서지가 생겼습니다). 빈 곳에 놓는 동작은 그대로 새 서지입니다.
...
- ⚠ 2026.9.4 이하로 저장한 문서나 그 버전으로 받는 쪽에서는 종전처럼 기록이 없어 '유령 인용'으로 보고됩니다 — 그때는 서재 폴더(또는 해당 docstore json)를 공유하거나 공용 클라우드 폴더를 씁니다.
...
- 부모가 **Vancouver 여도 파라미터가 반영됩니다**(2026.8.1) — 저자 축약·강조·DOI·쪽 표기가 먹습니다([5.1](#51-내장-엔진-스타일-6종)). 번호형[n]은 참고문헌에 반영됩니다.
...
- **KOSIS 통계표 주소로 담기 (2026.9.13)** — 같은 칸에 통계표 주소(`statHtml.do?orgId=…&tblId=…`)를 넣으면 **데이터세트 서지**가 됩니다(표명·작성기관·통계조사·수록 기간·인출일). KOSIS 화면에서 찾은 표를 앱으로 들여오지 않고도 인용으로 이을 수 있습니다.
...
- **한 건만 — [자동 이름] (2026.9.13)** — 이름 바꾸기 창의 **[자동 이름]** 을 누르면 **첫저자 - 연도 - 제목**을 칸에 채웁니다. 바로 바뀌지 않으니 보고 손볼 수 있습니다. 첨부 우클릭의 '자동 이름으로 바꾸기'도 함께 돌아왔습니다(국문 이름이 성만 남던 것도 고쳐서 '양 - 2025 - …' 이 '양연동 - 2025 - …' 이 됩니다).
...
- ⚠ **프록시는 미로그인 시 접근을 튕깁니다** — ⚙설정의 **[로그인 열기]** 로 먼저 한 번 로그인해 세션을 만드세요([9.4](#94-도서관-교외접속프록시-경유-설정)).
...
- **글자가 안 보이는 쪽이 있으면 그 쪽만 다시 그립니다 (2026.9.15)** — 제목 상자가 테두리만 그려지고 글자가 없던 문제입니다. 문서 잘못이 아니라 뷰어가 그 조합을 못 그리는 것이라 판을 올려도 안 풀렸습니다. 이제 **글자가 빠진 쪽을 알아채** 그 쪽만 Windows 내장 PDF 엔진으로 다시 그립니다(그 엔진은 이미 앱에 들어 있습니다 — 따로 받을 것이 없습니다). 글자 선택·찾기·주석은 그대로입니다.
- **글자가 밀려 뽑히던 PDF (2026.9.15)** — 화면은 멀쩡한데 **검색·AI 가 읽는 글자**가 어긋나 있는 문서가 있습니다(`Korean Journal of Sociology` 가 `,PSFBO+PVSOBM` 처럼). 조용히 틀리는 쪽이라 알아채기 어렵습니다. 이제 되돌릴 수 있는 것은 되돌리고, 되돌릴 수 없는 쪽은 **그 쪽만 다시 읽습니다**(요약·PDF 대화·쪽 찾기에서). 멀쩡한 문서는 한 글자도 바뀌지 않습니다.
...
**무엇을 뽑을지는 서지 유형에 따라 다릅니다 (2026.9.11)** — 종전에는 셋 다 실증 학술논문 하나만 전제해, 법령 PDF 에 표본 수를 묻고 판결에 방법론 타당성을 물었습니다. 이제 여섯 갈래로 갈립니다.

| 유형 | 뽑는 것 |
|---|---|
| 실증 논문 | 연구질문·방법·핵심결과·시사점/한계 (종전 그대로) |
...
- **스캔본도 읽습니다 (2026.9.2)** — 글자가 없는 PDF 는 Windows 내장 글자 인식으로 본문을 뽑아 넘깁니다. 종전에는 건너뛰고 OCR PDF 로 바꾸라고 안내만 했습니다.
  - ⏳ **처음 한 번은 시간이 걸립니다**(수십 쪽이면 십수 초~수 분). 묻지 않고 알아서 돕니다. 2026.9.16 부터 **시작하기 전에 먼저 알립니다** — 「스캔본입니다 — 글자를 먼저 읽습니다(42쪽). 긴 문서는 몇 분 걸릴 수 있습니다」(서지 채우기는 앞뒤 몇 쪽만 읽습니다). 그 뒤 진행이 **「스캔본이라 글자를 먼저 읽습니다 — 텍스트 인식 3/42쪽」** 으로 상태줄·진행 상자에 보입니다.
  - 한 번 읽어 두면 그 PDF 는 다음부터 바로 되고, **그때는 예고도 뜨지 않습니다** — 걸리지 않는 일을 걸린다고 말하지 않기 위해서입니다.
  - **AI 가 본문을 필요로 할 때만** 돕니다(요약·비판적 검토·PDF 대화·서지 채우기). **뷰어로 여는 것만으로는 돌지 않습니다** — 스캔본도 뷰어에서는 평소처럼 열립니다.
...
두 자리가 **같은 모양**이고(2026.9.15 — 종전에는 같은 제공자인데 폴백 자리에서만 모델 칸이 보였습니다), **'동시 실행'·'CLI 상태'는 맨 아래 «공통 · 1차·폴백 모두에 적용»** 로 갈라 두었습니다 — 종전에는 폴백 칸 안에 있어 폴백만의 설정처럼 보였습니다(폴백을 Ibis AI 로 고르면 특히).
...
- **체험 100 → 베타 무료 구독 (2026.9.14)** — 처음 시작하면 **체험 크레딧 100** 을 받습니다. 이것은 **다 쓰면 채워지지 않습니다.** 이어서 쓰려면 **[베타 무료 구독]** 을 시작하면 그 자리에서 **하루 50크레딧**이 매일 채워집니다 — 승인을 기다리지 않고 바로 시작됩니다.
  - **베타 기간 내내 요금이 없고 결제 수단도 받지 않습니다.** 정식 요금이 정해지면 구독하신 분들께 먼저 알리고, 그때 이어갈지 고르시면 됩니다.
  ...
  - 계정을 바꾸면 플랜과 남은 크레딧이 **즉시** 반영됩니다(2026.9.14 — 종전에는 최대 1분 동안 이전 계정 값이 배지·상태줄에 남았습니다).
- **Ibis AI 가 바쁠 때는 기다립니다 (2026.9.5)** — 여러 사람이 동시에 쓰면 서버 동시 한도에 걸려 'AI 제공자 오류(429)' 가 나던 것을, 자리가 날 때까지 잠깐씩(최대 90초) 기다렸다 다시 시도합니다. 기다리는 동안 상태줄에 보이고, 끝내 차례가 오지 않으면 그 사실을 말한 뒤 폴백 제공자가 있으면 그쪽으로 넘어갑니다. **'동시 실행'은 Ibis AI 에서 2건까지만** 적용됩니다.
...
> **도구 패널은 없앴습니다.** 그 안에 있던 것은 이렇게 갈라졌습니다 — 가져오기·내보내기·컬렉션 공유는 `파일` 메뉴, 일괄 정리(중복·원문·AI 태그·첨부 이름)는 `도구` 메뉴, DOI·ISBN 추가는 `새 문헌 추가` 창의 **식별자로 채우기**, 서재 위치·백업·업데이트 확인과 클라우드 로그인은 `⚙설정` 입니다. 예전 안내를 보고 도구 패널을 찾고 계셨다면 위 자리를 보세요.
...
- **Semantic Scholar** — 내장 키로 그냥 동작하지만 그 몫은 **초당 1회를 여럿이 나눠 씁니다**. 자주 쓰면 개인 키를 넣으세요(그때는 그 몫을 혼자 씁니다).
...
- **키를 하나도 넣지 않아도** 학술 DB 9종과 법·정책 5갈래가 전부 동작합니다. 위 세 칸은 있으면 나은 것이지 없으면 막히는 것이 아닙니다.
...
- ⚠ **종전에 경유 URL 은 넣고 체크란은 안 켜 두셨던 분은 이제 경유로 열립니다.** 교내 고정 IP 처럼 경유가 오히려 방해되는 드문 경우는 환경변수 `LIBRARY_PROXY_AUTO=0` 으로 끕니다(화면에는 없습니다).
...
- **'이 PC 의 서재' 목록 (2026.9.16)** — 이 PC 에서 써 본 서재가 **둘 이상일 때** 이 탭에 목록이 생깁니다(하나뿐이면 안 보입니다). 줄마다 **서지 건수**가 함께 적혀 있어 어느 쪽이 쓰던 서재인지 숫자로 가릅니다. [열기] 한 번으로 바꿉니다 — 위 로그를 찾아 읽지 않아도 됩니다.
- **빈 서재에서 내 서재 동기화를 켜면 먼저 묻습니다 (2026.9.16)** — 서버 사본이 내려와 채워지지만 **첨부 PDF 는 따라오지 않습니다**. 원래 쓰던 서재가 이 PC 에 있다면 그쪽을 여는 것이 맞아서, **[그 서재 열기]** 와 **[여기서 켜기]** 를 고르게 합니다(막지는 않습니다).
...
**글꼴** — 2026.9.5 부터 화면 글꼴은 **Pretendard 하나**입니다. 글꼴 선택과 내려받기(본명조·나눔·JetBrains Mono)를 뺐고, 종이톤 테마 둘도 Pretendard 로 맞췄습니다(여러 글꼴을 지원할 이유가 없다는 판단). Pretendard 를 못 쓰는 환경에서는 맑은 고딕으로 내려갑니다. 예전에 받아 둔 글꼴 파일(`%LOCALAPPDATA%\Ibis\fonts`)은 더 쓰이지 않으니 지워도 됩니다.
