...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.5**.
...
| **레일**(맨 왼쪽) | 서재 · 외부 검색 · **질문으로 찾기** · 그래프 뷰 전환. 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 참고문헌 갱신 · **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래) |
...
- **첨부 PDF 본문까지 (2026.9.5)** — '전체' 검색이 첨부 PDF 의 글자까지 봅니다. 제목·초록에 없는 낱말을 PDF 가 품고 있으면 그 문헌이 뒤에 덧붙고 행 아래 **발췌 한 줄**(일치 부분 강조)이 보입니다. 고급 검색에 **본문(PDF)** 필드가 생겼습니다. 색인은 첨부 뒤 배경에서 저절로(부팅 뒤·첨부 뒤 1분 안) 만들어지고, 상태와 [지금 색인]은 `⚙설정 › 서재 › PDF 본문 색인` 에 있습니다. 스캔본은 글자 인식 뒤에 잡힙니다.
