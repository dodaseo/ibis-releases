...
한국 연구자를 위한 서지·인용 관리 앱. **국내 학술 검색 → 원문 확보 → 읽기·AI 분석 → 태그 그래프 → 본문 인용(CWYW)** 까지 한 프로그램에서 끝냅니다. 기준 버전 **2026.7.1**.

- 표기: `⚙설정` · `🔧도구` · `새 문헌 추가` 는 화면의 실제 라벨입니다.
- 좌측 세로 아이콘 줄 = **레일**, 상단의 '○○ 모드 · 문서명' = **상태 배지**.
- 버전은 CalVer(`연.월.패치`) — `0.7.2` 다음이 `2026.7.0`·`2026.7.1` 입니다.
...
| 파일 | 용도 |
|---|---|
| `Ibis-<버전>-setup.exe` | **설치 프로그램**(권장) — 관리자 권한 불필요, 시작 메뉴·바탕화면 아이콘, 제어판에서 제거 |
| `Ibis-<버전>-win64.zip` | **무설치** — 압축을 풀고 `Ibis.exe` 실행 |

- 설치 위치: `%LOCALAPPDATA%\Programs\Ibis`(설치 중 변경 가능).
- ⚠ **`C:\Program Files` 로 바꾸지 마세요** — 인앱 업데이트가 이 폴더를 직접 교체하므로 쓰기 권한이 없으면 업데이트가 실패합니다.
- 첫 실행 시 SmartScreen 경고(코드 서명 없음) → `추가 정보 → 실행`. 릴리스의 `.sha256` 으로 무결성 확인 가능.
- 서지 데이터는 설치 폴더가 아니라 `%LOCALAPPDATA%\hwp-endnote` 에 있어 **제거·재설치·업데이트로 지워지지 않습니다.**

### 1.3 최초 실행 온보딩

'Ibis 시작하기' 창이 뜹니다. **전부 선택 사항**이며 [나중에]로 건너뛸 수 있습니다.

1. **라이브러리 위치** — 기본은 `%LOCALAPPDATA%\hwp-endnote`. 클라우드 폴더로 지정하면 여러 PC 동기화.
2. **학술 DB 연동** — KCI·ScienceON 인증키, 이메일(OA).
3. **AI 제공자** — 구독 CLI 또는 API 키.

- 건너뛴 항목은 언제든 `⚙설정`·`🔧도구` 에서 설정 가능.
...
| **레일**(맨 왼쪽) | 라이브러리 · 외부 검색 · 그래프 뷰 전환, 아래쪽에 참고문헌 갱신 · 휴지통 |
...
- **'새 문헌 추가'**(사이드바 맨 위)는 폼을 바로 열지 않고 **추가 방법 메뉴**를 띄웁니다 — 새 문헌 직접 추가·인용문 붙여넣기 / 파일로 서지 가져오기(RIS·BibTeX·CSL) / 국내 학술 DB 검색(KCI·ScienceON) / 해외 학술 DB 검색(OpenAlex·Crossref).
...
5. 앱 시작 시 이미 떠 있으면 활성 문서에 자동 연결(한글 우선).
...
| 소스 | 커버리지 | 키 |
|---|---|---|
| KCI | 국내 학술·학위논문 | 필요(인증키) |
| ScienceON | 국내 학술·학위논문(KISTI) | 필요(3개 값) |
| Crossref | 국제+국내 상당수 | 불필요(기본값) |
| OpenAlex | 국제+국내 상당수 | 불필요 |
| Semantic Scholar | CS·융합 | 선택(넣으면 속도·쿼터↑) |
| arXiv | 프리프린트 | 불필요 |
| 내 서지 | 보관한 문헌 | — |

- 기본 선택 **Crossref**, 키가 없으면 '내 서지'.
- KCI·ScienceON 은 **키가 설정돼 있을 때만** 드롭다운에 표시됩니다.
...
- 외부 소스가 2개 이상 가능할 때만 **'⚡멀티'** 표시.
- 키가 있는 KCI·ScienceON + 무료 4종을 **동시 질의**.
- 병합 기준: **DOI 우선**, 없으면 정규화 제목+연도 → 출처를 누적 표시.
- 일부 소스가 실패(키 미설정·429)해도 나머지는 진행 → 제목줄에 `멀티 검색 · 실패: 소스명`.

### 3.3 DOI 즉시 서지화

- **방법 A** — 검색창에 DOI(또는 doi.org URL) 붙여넣기 → 소스 무관하게 1건 해석('내 서지' 소스만 예외).
- **방법 B** — `🔧도구` 의 'DOI로 추가' 칸 → 해석 후 바로 내 서지에 보관.
...
- **추가** — `🔧도구` 의 'ISBN으로 추가' 칸에 ISBN(하이픈 있어도 됨) 입력 → '추가'. 권위 조회 후 단행본 서지가 바로 내 서지에 보관됩니다.
...
- **국내서는 국립중앙도서관 인증키**가 있어야 정확합니다 — 해외서는 키 없이 OpenLibrary 로 조회됩니다(키·발급은 [9.1](#91-api-키이메일-발급)).
...
- 지정 항목: 소스 · 검색 필드(전체/제목/저자/초록/키워드) · 저자 · 연도 범위 · 학술지 · 정렬 · 문서 유형.
- 빈 칸은 조건에서 제외, 적용된 조건은 결과 제목줄에 요약 표시.

소스별 지원 범위:

- **ScienceON** — 전체/제목/저자/초록/키워드를 실제 필드로 매핑.
- **KCI** — 제목·저자·키워드·학술지(초록·전체는 제목검색으로 처리).
- **Crossref·OpenAlex·S2·arXiv** — 제목/저자/연도 중심(전용 초록 필드 없음).
- **문서 유형·연도 범위** — 결과 내 후필터로 정제(단일 연도만 소스에 위임).
...
- 경로: `🔧도구` › '가져오기 (RIS·BibTeX·CSL)' → 파일 선택(복수 가능).
...
- **RIS·BibTeX·XML 등 서지 파일** — 임포터로 서지를 가져옵니다([3.9](#39-파일-가져오기) 와 동일 처리).
...
- 파일 **자체**를 추가하는 경로입니다 — 완성형 인용문 **텍스트**를 붙여 파싱하는 것은 아래 [3.11](#311-인용문-붙여넣기-파싱) 입니다.
...
2. 문헌 선택 → 상세 패널에서 **형태**(괄호형 `(저자, 연도)` / 서술형 `저자(연도)` / 연도만 `(연도)`)와 **'쪽'** 지정.
3. **'인용 삽입'** 클릭.
4. 삽입 즉시 문서 전체가 현재 스타일로 재서식되어 최종 표기 확정.

- 아직 내 서지에 없는 문헌은 자동 보관됩니다.
- **서술형은 단일 문헌일 때만** 적용(여러 건이면 괄호형으로 폴백).
- 삽입 순간에는 임시로 저자-연도가 보이고, 곧바로 실행되는 전체 갱신에서 확정됩니다.
- 웹판 UI 는 형태·쪽까지만 노출합니다(접두/접미 문구는 코어에 있으나 화면 버튼 없음).

### 4.2 병렬 인용 — 여러 문헌을 한 괄호로
...
| **Vancouver** | 번호형 [n] 등장순, 저자 최대 6인+et al. **파라미터 무시(고정 서식)** |
...
- 기존 구간(마커~마커)을 통째로 교체하므로 중복이 생기지 않습니다.
- 제목('참고문헌')은 **기본으로 넣지 않고** 스타일의 `bib_heading` 지정 시에만.
- 재구축 중 한글의 '누름틀 삭제' 팝업은 자동 응답 후 원복합니다.
...
- 내장 프리셋 **교육사회학연구(KJSE)** 제공(부모 APA, 4인↑ 축약, 구분자 ㆍ, 무간격, 쪽 `1999: 34`, 논문제목 따옴표, 국문 볼드/영문 이탤릭, DOI 미표기).
...
- 부모가 **Vancouver 면 파라미터 무시**, 번호형[n]은 참고문헌에만 반영. 세부 파라미터는 대부분 **APA 부모에서만** 의미가 있습니다.
...
- 라이브러리 폴더를 클라우드에 두면 여러 PC 공유([9.6](#96-라이브러리-위치클라우드-동기화)).
...
1. 사이드바 **'새 문헌 추가' › '새 문헌 직접 추가·인용문 붙여넣기'**(또는 `🔧도구 › 문헌 추가 › 직접 입력·인용문 붙여넣기`).
...
- **중복 정리** — `🔧도구` › '중복 정리'. DOI 또는 정규화 제목+연도가 같은 문헌을 **가장 충실한 1건**에 병합(빈 필드 보강·첨부/키워드 합침·컬렉션 이관·★·이른 추가일 승계).
- **첨부(PDF)** — 우클릭 › '원문 확보'로 자동 첨부([7절](#7-원문-확보pdf-뷰어)). 서지 기반 자동 개명('첫저자 - 연도 - 제목'), 오첨부는 '다른 문헌으로 이동'.
- **첨부 자동 이름 일괄 정리** — `🔧도구` › '첨부 자동 이름 정리'. 라이브러리 전체 첨부를 서지 기반 파일명으로 한 번에 정리하고 결과(정리·유지·실패)를 브리핑합니다.
...
1. `⚙설정` 에 **'도서관 경유 URL'** 입력(예: `https://proxy.○○.ac.kr/_Lib_Proxy_Url/`).
...
- **PDF 판본이 바뀌면** — 주석을 단 뒤 첨부 PDF 가 교체·변경되면 좌표가 어긋날 수 있어 주석 목록·상태줄에 **'⚠ PDF 가 바뀌어 위치가 어긋날 수 있음'** 이 표시됩니다. 주석은 선택 앞뒤 문맥까지 저장해 재정렬을 시도합니다.
...
> AI 는 앱이 비용을 부담하지 않고 **사용자 본인의 자원**(구독 CLI 또는 API 키)으로 동작합니다. 설정하지 않으면 AI 기능만 비활성이고 나머지는 그대로 됩니다.
...
- 본문 텍스트가 없으면(스캔본·워터마크) 건너뛰고 OCR PDF 로 교체하라고 안내합니다.
...
- **모델** — **'모델 조회'** 버튼으로 그 제공자의 사용 가능 모델을 불러와 드롭다운에서 고릅니다(직접 입력도 가능). 비우면 CLI 개인 기본값 / API 내장 기본값(Anthropic `claude-haiku-4-5` · OpenAI `gpt-4o-mini` · Gemini `gemini-2.0-flash`).
- **추론 강도(선택)** — 지원하는 모델에서 추론(사고) 강도를 고릅니다.
- **폴백 제공자(선택)** — 1차 실패 시 지정 제공자로 1회 자동 재시도(폴백도 제공자·모델·추론 강도를 따로 지정).
...
`⚙설정`(톱니)은 **일반 · AI · 외부 API** 3개 탭으로 구성됩니다.

- **일반 탭** — 문서 연결([2절](#2-한글word-문서-연결)) · 인용 스타일([5절](#5-참고문헌인용-스타일)) · 표시(테마·글꼴·항상 위 — 9.7).
- **AI 탭** — AI 제공자·모델·추론 강도·폴백(9.3 · [8.4](#84-ai-제공자모델폴백)).
- **외부 API 탭** — 학술 DB 키·이메일([9.1](#91-api-키이메일-발급)) · 도서관 원문([9.4](#94-도서관-교외접속프록시-경유-설정)).

라이브러리 이전·가져오기/내보내기·중복 정리·PDF 일괄 확보는 `🔧도구`(도구 아이콘)에 따로 있습니다([9.6](#96-라이브러리-위치클라우드-동기화)). 설정 칸은 대부분 **'빈 칸은 유지' 규칙**이라 비우고 저장해도 지워지지 않습니다(삭제는 '지움' 버튼).

### 9.1 API 키·이메일 발급

*(⚙설정 › 외부 API 탭. 아래 KCI·ScienceON 은 검색용, 국립중앙도서관·Google Books 는 단행본 ISBN 조회용.)*

![설정 — 외부 API 탭(학술 DB 키·ISBN 조회·도서관 경유)](images/settings-api.png)

| 항목 | 발급처 · 절차 | 입력 칸 | 문의 |
|---|---|---|---|
| **KCI 인증키** | `open.kci.go.kr` → KCI 데이터 제공 › OPEN API › OPEN API 키신청(공용 단일 키) | `KCI 인증키` | kciadmin@nrf.re.kr · 042-869-6674 |
| **ScienceON** | `scienceon.kisti.re.kr` 로그인 → 인증키 발급 신청 → **Client ID·인증키·MAC 3개** | `ScienceON ID·Key·MAC` | helpdesk@kisti.re.kr · 080-969-4114 |
| **이메일(OA·polite)** | 발급 아님. 본인 이메일(Unpaywall + Crossref/OpenAlex polite pool) | `이메일(OA·polite)` | — |
| **국립중앙도서관** | `seoji.nl.go.kr` → 인증키 신청(단행본 ISBN 권위조회) | `국립중앙도서관` | — |
| **Google Books** | 발급 선택 — Google Cloud 콘솔의 Books API 키(ISBN 조회 보조) | `Google Books` | — |

- **KCI** — 저장하면 검색 소스에 KCI 가 나타납니다.
- **ScienceON** — **세 값이 모두** 있어야 동작(MAC 하나만 빠져도 검색 불가).
- **이메일 칸은 예외** — 비우고 저장하면 삭제됩니다. Unpaywall·Crossref 두 값에 같은 주소가 저장됩니다.
- **국립중앙도서관·Google Books** — 단행본 **ISBN 조회**용(선택, [3.4](#34-isbn으로-단행본-추가)). 국내서는 국립중앙도서관 키가 있어야 정확하고, 해외서는 키 없이 OpenLibrary 로 조회됩니다. Google Books 는 키 없이 쓰면 공용 쿼터 초과가 흔해 보조로만 씁니다.
- 키 없이도 Crossref·OpenAlex·S2·arXiv 는 무키로 검색됩니다.

### 9.2 저장된 키 삭제 — '지움'

1. 설정된 키는 `● 설정됨` 으로 표시되고 옆에 **'지움'** 버튼이 나타납니다.
2. '지움' → **'저장'** → 실제 삭제되고 해당 소스도 드롭다운에서 사라집니다.

- 환경변수(`KCI_API_KEY`·`SCIENCEON_*` 등)로 설정된 값은 외부 구성이라 UI 에서 지워지지 않습니다.
...
| 항목 | 기본 위치 | 클라우드 동기화 |
|---|---|---|
| 라이브러리(서지) | `%LOCALAPPDATA%\hwp-endnote\library.json` | 폴더를 클라우드로 지정 시 O |
...
| 받은 글꼴 | `%LOCALAPPDATA%\hwp-endnote\fonts\` | X (PC 로컬) |
| 비밀정보(API 키·테마·글꼴·AI) | `%LOCALAPPDATA%\hwp-endnote\secrets.json` | **X (항상 로컬)** |

- 비밀정보는 권한 `0o600` 으로 저장되며 항상 로컬 — **클라우드 라이브러리를 써도 API 키·테마·글꼴·AI 설정은 PC 마다 따로** 넣어야 합니다.
- **환경변수가 `secrets.json` 보다 우선**합니다(`KCI_API_KEY`·`SCIENCEON_*`·`UNPAYWALL_EMAIL`·`LIBRARY_PROXY_PREFIX/AUTO`·`LIBRARY_LOGIN_URL`·`HWPENDNOTE_LIBRARY` 등).
- 라이브러리 경로 우선순위: env `HWPENDNOTE_LIBRARY` > secrets `library_path` > 기본.
- `🔧도구 › 라이브러리 › '라이브러리 폴더 열기'` 로 탐색기에서 확인.

### 9.6 라이브러리 위치·클라우드 동기화
...
**테마 16종** — 고르는 즉시 열려 있는 모든 창에 적용되고 저장됩니다.
...
| | |
|---|---|
| ![Slate](images/theme-slate.png) Slate — 기본 | ![Ivory](images/theme-ivory.png) Ivory — 종이톤·명조 |
...
- **받아서 쓰는 글꼴**: 본명조(Noto Serif KR) · 나눔고딕 · 나눔명조 · 나눔고딕코딩 · JetBrains Mono.
  - 전부 SIL Open Font License 1.1 이며 **설치 파일에는 없습니다** — '글꼴 받기' 에서 필요한 것만 공식 배포처에서 내려받습니다(라이선스 사본도 함께 저장).
  - 보관 위치: `%LOCALAPPDATA%\hwp-endnote\fonts` — 업데이트해도 남습니다.
...
- 모든 항목(라이브러리 위치·KCI·ScienceON·이메일·도서관 경유·AI)은 `⚙설정`·`🔧도구` 에서 언제든 변경 가능.
...
- **API 키 없이 쓸 수 있나요?** — 예. 검색(Crossref·OpenAlex·S2·arXiv)·인용 삽입·서지관리·스타일 모두 가능. KCI·ScienceON 만 키 필요.
