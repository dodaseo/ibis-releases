...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.12**.
...
- **상세**(서지 상세와 다른 화면) — 표명·작성기관·조사·수록 기간 → **[수치 보기]**(수록 주기·항목·최근 N시점을 고르면 **시점 × 분류·항목** 표) → 분류 경로·표 주석 → 인용 → 서지 정보. 읽는 화면이라 피벗·차트는 [KOSIS 에서 보기]로, 원자료는 **[CSV 저장]**(분류 코드·이름·항목·단위·시점·값)으로 받습니다. 한 번에 4만 셀까지(KOSIS 제한) — 넘으면 시점 수나 항목을 좁히라고 알립니다.
...
- **고급 검색은 범위를 따릅니다**(2026.9.12) — 법·정책과 통계는 두 API 다 검색어 하나만 받으므로 조건 줄이 하나(필드 '전체' 고정)이고 정렬 줄은 없습니다(관련도순뿐). 통계는 유형 줄 대신 **작성기관·통계조사** 칸(받은 목록 안에서 거름)과 **수록 연도**, 법·정책은 **공포·제안 연도**를 씁니다.
