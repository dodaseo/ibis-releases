...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.11**.
...
| **레일**(맨 왼쪽) | 서재 · **키워드로 찾기** · **질문으로 찾기** · 그래프 뷰 전환. 세 모드가 **같은 서재 사이드바**를 쓰고(어느 모드에서든 컬렉션을 누르면 그 뷰로 갑니다), 찾기 줄과 그 아래 DB 체크 행도 같은 자리입니다(2026.9.8). 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래). 레일의 참고문헌 갱신(⟳) 아이콘은 2026.9.6 에 뺐습니다 — 새로고침으로 오해돼서요. 갱신은 [4.5](#45-전체-갱신--참고문헌-갱신) 의 자리에서 합니다 |
...
낱말이 아니라 **연구 질의를 문장으로** 던지면, 국내외 학술 DB 를 뒤져 **관련도 순위**로 돌려줍니다. 레일 **세 번째 아이콘**입니다.
...
2. 찾기 줄 아래 **체크 행**에서 어디서 찾을지 고릅니다: `전체` │ **내 서재** │ **국내**(KCI·ScienceON·국립중앙도서관) │ **국외**(OpenAlex·Crossref·ERIC·arXiv·PubMed). 행 오른쪽 끝에 **예상 크레딧**(`+2~7` 처럼)이 적힙니다.
   - **내 서재 칸은 기본이 꺼짐**입니다(2026.9.10) — 꺼져 있으면 DB 에서만 찾고, 그중 이미 담아 둔 것에는 **[보관됨]** 배지가 붙습니다. 켜면 서재 문헌도 후보로 순위에 함께 듭니다.
...
>
> 두 찾기는 **같은 자리(찾기 줄 아래)에서 같은 방식**으로 DB 를 고릅니다 — 묶음 이름을 누르면 그 묶음이 통째로 켜지고 꺼지며 조합은 자유입니다. 다만 **목록은 다릅니다**: 키워드로 찾기에는 **법·정책 다섯 갈래**와 S2 가 있고, 질문으로 찾기에는 **내 서재** 칸이 있습니다(기본 꺼짐 — 켜면 서재 문헌도 순위에 함께 듭니다). 고른 조합은 키워드 쪽만 기억됩니다.

### 3.2 검색 소스

레일 **두 번째 아이콘 [키워드로 찾기]** 입니다. 어디서 찾을지는 **찾기 줄 바로 아래 체크 행**에서 고릅니다 — `전체 │ 국내 KCI·ScienceON·국중도 │ 국외 OpenAlex·Crossref·ERIC·arXiv·PubMed·S2 │ 법·정책 법령·판례·정책자료·법률안·회의록`(arXiv·PubMed·S2 와 법·정책 다섯은 **기본 꺼짐**). **'국내'·'국외' 글자를 누르면 그 묶음이 통째로 켜지고 꺼지며**, 원하는 대로 섞어도 됩니다(KCI+ERIC 만 같은 조합도 가능). 고른 조합은 기억되고, 행 오른쪽 끝에 `○○에서 찾기` 라고 지금 조합이 적힙니다(고른 것이 없으면 'DB 를 하나 이상 고르세요').
...
| — | 내 서지 | 보관한 문헌 — 키워드로 찾기의 체크 항목은 아닙니다. **서재 모드의 찾기 줄**에서 찾고([3.8](#38-내-서지-전문검색fts)), 질문으로 찾기에는 '내 서재' 칸이 있습니다([3.1](#31-질문으로-찾기-202691)). 찾은 결과가 이미 서재에 있으면 행에 **'보관됨'** 배지가 붙습니다 | — |
...
- **유형(공통)** · 정렬 · 발행년 범위. 유형에 **법령·조례**가 있습니다(2026.9.9 — 담기고 인용도 되는데 고를 수가 없었습니다). (어느 DB 에서 찾을지는 고급 검색 안이 아니라 **찾기 줄 아래 체크 행**입니다.)
