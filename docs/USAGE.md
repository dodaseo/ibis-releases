...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.6**.
...
| **레일**(맨 왼쪽) | 서재 · 외부 검색 · **질문으로 찾기** · 그래프 뷰 전환. 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래). 레일의 참고문헌 갱신(⟳) 아이콘은 2026.9.6 에 뺐습니다 — 새로고침으로 오해돼서요. 갱신은 [4.5](#45-전체-갱신--참고문헌-갱신) 의 자리에서 합니다 |
...
- **사이드바를 누르면 질문 화면을 닫고 그 뷰로 갑니다 (2026.9.6)** — 외부 검색 모드와 같은 동작입니다. 결과는 남아 있어 레일의 [질문으로 찾기]로 돌아오면 그대로이고, 결과 행을 컬렉션에 끌어 넣는 것도 그대로입니다. Ibis AI 를 켠 직후에도 '⚙설정에서 켜 주세요'가 뜨던 문제도 함께 고쳤습니다.
...
소스는 **국내 3종 · 해외 6종** 으로 나뉘고, 각 묶음에 **'국내 통합'·'해외 통합'** 이 있어 묶음 안을 병렬로 질의한 뒤 중복을 합칩니다.
