...
한국 연구자를 위한 서지·인용 관리 앱. **국내 학술 검색 → 원문 확보 → 읽기·AI 분석 → 태그 그래프 → 본문 인용(CWYW)** 까지 한 프로그램에서 끝냅니다. 기준 버전 **2026.8.7**.
...
[릴리스 페이지](https://github.com/dodaseo/ibis-releases/releases/latest)에서 둘 중 하나를 받습니다.
...
- 서지 데이터는 설치 폴더가 아니라 `%LOCALAPPDATA%\Ibis` 에 있어 **제거·재설치·업데이트로 지워지지 않습니다.**
...
- 버튼은 `나중에` / `저장하고 시작`.
- 검색은 온보딩에서 설정할 것이 없습니다 — KCI·국립중앙도서관은 앱 내장 키로 바로 동작합니다([3.1](#31-검색-소스)).
...
| **레일**(맨 왼쪽) | 라이브러리 · 외부 검색 · 그래프 뷰 전환, 아래쪽에 참고문헌 갱신 · **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래) |
...
- **`🔧도구` 와 `⚙설정` 의 경계** — 도구에는 *문헌을 다루는 일*(가져오기/내보내기·중복 정리·PDF 일괄 확보·첨부 이름 정리·AI 태그 정리)이, 설정에는 *환경을 정하는 일*이 있습니다. **라이브러리 위치·백업·업데이트는 2026.8.7 에 도구에서 `⚙설정 › 라이브러리` 탭으로 옮겨졌습니다**([9절](#9-설정api-키)).
...
1. 사이드바 **'새 문헌 추가' › `새 문헌 추가 — DOI·ISBN·인용문·직접 입력…`**(Ctrl+N, 또는 `🔧도구 › 문헌 추가`).
...
- 우클릭 **[참가자·초대 관리…]** 로 누가 들어와 있는지 보고 잘못 나간 초대 링크를 끊습니다. **[활동 기록…]** 은 누가 언제 무엇을 바꿨는지 보여 줍니다.
- **그만두기** — 우클릭 **[클라우드 공유 해제…]**. 소유자는 *서버에서 없애기* 와 *이 PC 연결만 끊기* 중에 고르고, 참가자는 **[공유에서 나가기]** 입니다.
- 상태줄에 `☁ 사용량 / 50MB`(베타 할당)와 **[동기화]** 버튼이 보입니다.
...
1. **두 사람이 같은 문헌의 '이미 채워진 같은 칸'을 다르게 고치면 그 교정은 전파되지 않습니다.** 받는 쪽 규칙이 '빈 칸만 채운다' 라서입니다. 새 문헌·빈 칸 채우기는 정상으로 오갑니다.
2. **컬렉션의 이름·색·정렬 순서를 바꾼 것도 전파되지 않습니다.** 참가자마다 다른 이름으로 굳는 것을 막기 위해 일부러 뺐습니다(넣으면 서로 되밀기를 반복합니다).
...
**지금 읽는 논문 한 편만**을 근거로 묻고 답합니다. 이어서 되물을 수 있습니다.
...
- 답변 본문의 **`(p.쪽)` 표기 자체가 버튼**이라 누르면 그 근거 자리로 갑니다. 쪽 번호는 모델이 적은 것을 믿지 않고 앱이 원문에서 되찾아 확정합니다.
...
`⚙설정`(톱니)은 **일반 · AI · 외부 API · 클라우드 · 라이브러리** 5개 탭입니다. 레일 맨 아래 ⚙ 로 엽니다.
...
| **외부 API** | ScienceON 인증키·이메일([9.1](#91-api-키이메일-발급)) · 도서관 원문([9.4](#94-도서관-교외접속프록시-경유-설정)) |
| **클라우드** | 구글 로그인 · 공유 중인 컬렉션 목록 · 서버 사용량([6.10](#610-컬렉션-공유)) |
| **라이브러리** | 라이브러리 위치·백업·업데이트 확인([9.6](#96-라이브러리-위치클라우드-폴더-동기화)) |

> **'라이브러리' 탭은 2026.8.7 에 새로 생겼습니다.** 종전에 `🔧도구` 패널에 있던 라이브러리 위치·백업·업데이트가 이리로 옮겨졌습니다. `🔧도구` 에는 문헌을 다루는 일(가져오기/내보내기·중복 정리·PDF 일괄 확보·첨부 이름 정리·AI 태그 정리)만 남았습니다.

설정 칸은 대부분 **'빈 칸은 유지' 규칙**이라 비우고 저장해도 지워지지 않습니다(삭제는 '지움' 버튼).

### 9.1 API 키·이메일 발급

*(⚙설정 › 외부 API 탭. **KCI·국립중앙도서관은 앱 내장 키로 동작해 입력 칸이 없습니다** — 아래가 남은 전부입니다.)*
...
- **Google Books** — 단행본 **ISBN 조회** 보조용(선택, [3.4](#34-isbn으로-단행본-추가)). 키 없이 쓰면 공용 쿼터 초과가 흔해 보조로만 씁니다. ISBN 권위조회 자체는 내장 키로 국립중앙도서관을 씁니다.
...
`⚙설정 › 라이브러리` 탭(2026.8.7 이전에는 `🔧도구` 에 있었습니다).
...
- 모든 항목(라이브러리 위치·이메일·AI, 그리고 온보딩에 없는 ScienceON·도서관 경유)은 `⚙설정`·`🔧도구` 에서 언제든 변경 가능.
