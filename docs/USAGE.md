...
결과는 **키워드로 찾기와 같은 표**에 나옵니다(2026.9.8 — 그전에는 별도 카드 목록이었습니다). 맨 왼쪽에 **관련도 칸**(순위 · 막대 · 0~1 점수)이 하나 더 붙고, 그 칸은 질문으로 찾기에서만 보입니다. 최근 질의·보낸 검색어·사정 고지는 표 바로 위 한 줄에 있습니다.
...
4. 안내줄의 **검색어**를 누르면 그 검색어로 키워드로 찾기가 바로 돕니다(2026.9.12 — 질문 = 탐색, 키워드 = 확정). 최근 질의는 셋만 보이고 나머지는 **[+N ▾]** 에 접혀 있습니다.
...
- **질문** — "초등학생 1인당 월평균 사교육비는 얼마나 되나?" 처럼 문장으로 물으면 검색어를 만들어 표를 모으고, **조사 개요(정의·목적) + 표명·조사·분류 경로 + 항목·분류 이름 + 표 주석**으로 관련도를 매깁니다. 상위 5개 표는 **최근 시점 수치**가 행에 📊 발췌로 미리 보입니다(열기 전에 숫자를 봅니다). 크레딧은 학술 질문과 같습니다.
