...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.15**.
...
- **첨부 PDF 본문까지 (2026.9.5)** — '전체' 검색이 첨부 PDF 의 글자까지 봅니다. 제목·초록에 없는 낱말을 PDF 가 품고 있으면 그 문헌이 뒤에 덧붙고 행 아래 **발췌 한 줄**(일치 부분 강조)이 보입니다. 고급 검색에 **본문(PDF)** 필드가 생겼습니다. 색인은 첨부 뒤 배경에서 저절로(부팅 뒤·첨부 뒤 1분 안) 만들어지고, 상태와 [지금 색인]은 `⚙설정 › 서재 › PDF 본문 색인` 에 있습니다. 스캔본은 글자 인식 뒤에 잡힙니다. 색인은 **낮은 우선순위로 파일 사이에 쉬며** 돌고, 상태줄에 `n/전체` 진행이 보입니다(2026.9.6 — 첫 색인 수백 편이 도는 동안 창이 굼뜨던 문제).
...
- **스캔본도 읽습니다 (2026.9.2)** — 글자가 없는 PDF 는 Windows 내장 글자 인식으로 본문을 뽑아 넘깁니다. 종전에는 건너뛰고 OCR PDF 로 바꾸라고 안내만 했습니다.
  - ⏳ **처음 한 번은 시간이 걸립니다**(수십 쪽이면 십수 초~수 분). 묻지 않고 알아서 돌고, 진행이 **「스캔본이라 글자를 먼저 읽습니다 — 텍스트 인식 3/42쪽」** 으로 상태줄·진행 상자에 보입니다. 한 번 읽어 두면 그 PDF 는 다음부터 바로 됩니다.
