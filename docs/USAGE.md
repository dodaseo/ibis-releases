...
한국어로 연구하는 사람을 위한 **문헌 관리 앱**. **찾기 → 원문 확보 → 읽기·AI 분석 → 정리·태그 그래프 → 동료와 나누기** 까지 한 프로그램 안에서 합니다. 아래아한글·Word 본문 인용(CWYW)도 그중 한 기능입니다. 기준 버전 **2026.9.7**.
...
| **레일**(맨 왼쪽) | 서재 · **DB 검색** · **질문으로 찾기** · 그래프 뷰 전환(2026.9.7 부터 '외부 검색'을 **DB 검색**이라 부르고, 세 모드가 **같은 서재 사이드바**를 씁니다 — 어느 모드에서든 컬렉션을 누르면 그 뷰로 갑니다). 맨 아래에 **계정 동그라미**(로그인하면 구글 사진), 아래쪽에 **인용 편집 창** · 휴지통 · **⚙설정**(맨 아래). 레일의 참고문헌 갱신(⟳) 아이콘은 2026.9.6 에 뺐습니다 — 새로고침으로 오해돼서요. 갱신은 [4.5](#45-전체-갱신--참고문헌-갱신) 의 자리에서 합니다 |
...
- **사이드바를 누르면 질문 화면을 닫고 그 뷰로 갑니다 (2026.9.6)** — DB 검색 모드와 같은 동작입니다. 결과는 남아 있어 레일의 [질문으로 찾기]로 돌아오면 그대로이고, 결과 행을 컬렉션에 끌어 넣는 것도 그대로입니다. Ibis AI 를 켠 직후에도 '⚙설정에서 켜 주세요'가 뜨던 문제도 함께 고쳤습니다.
...
> 낱말로 좁혀 찾는 것은 **DB 검색**([3.2](#32-검색-소스))입니다. 질문으로 찾기는 *무엇을 찾아야 할지부터 넓게 훑을 때* 씁니다.

### 3.2 검색 소스

레일의 **DB 검색**(2026.9.7 부터의 이름 — 그전에는 '외부 검색')입니다. 어디서 찾을지는 **검색줄 바로 아래 체크 행**에서 고릅니다(질문으로 찾기와 같은 모양) — `전체 │ 국내 KCI·ScienceON·국중도 │ 국외 OpenAlex·Crossref·ERIC·arXiv·PubMed·S2`(arXiv·PubMed·S2 는 주제 범위가 좁아 **기본 꺼짐**). **'국내'·'국외' 글자를 누르면 그 묶음이 통째로 켜지고 꺼지며**, 원하는 대로 섞어도 됩니다(KCI+ERIC 만 같은 조합도 가능). 고른 조합은 기억되고, 오른쪽 끝에 `○○에서 검색` 이라고 지금 조합이 적힙니다.
...
| — | 내 서지 | 보관한 문헌 — 2026.9.7 부터 DB 검색의 체크 항목이 아닙니다. 내 문헌은 **서재 모드의 검색줄**에서 찾습니다([3.8](#38-내-서지-전문검색fts)) | — |
...
- 검색: **체크 행에서 DB 고르기 → 검색어 입력 → Enter** → 페이지당 20건. 체크를 바꾸면 목록이 새 조합으로 바뀌고, 실제 검색은 Enter 로 합니다(질문으로 찾기와 같은 결).
...
![DB 검색 — 검색줄 아래 DB 선택 행, 결과에 출처 배지](images/search.png)
